...
| A burn-deposit (Bitcoin → Ethereum) reveal won't relay | Its envelope and the note it burns are locked to the same Bitcoin input by the guest, which pushes the witness item carrying the envelope well past Bitcoin Core's 80-byte standardness cap — ordinary relay refuses it even though it's fully consensus-valid. | Broadcast it via `makeBurnDepositBroadcaster` (`dapp/burndep-broadcast.js`), which submits to MARA Slipstream instead of p2p relay, then registers the provenance once real confirmation is observed. See [`BUILD-A-TACIT-DAPP.md` §5f](./BUILD-A-TACIT-DAPP.md#5f-bridging-between-chains). |
