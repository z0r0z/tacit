...
| A `crossOut()`'s Bitcoin-side mint never folds | `crossOut()` only settles the Ethereum side; the Bitcoin-side reveal is a separate step, and the guest checks it once, at scan time, against its current Ethereum-state view — a claim broadcast before that view includes the crossOut is a permanent miss, not a retry. | Use `broadcast.completeCrossOutOnBitcoin({ block: r.ethBlock, ...r.crossOuts[0] })` (`dapp/crossout-broadcast.js`) — it waits on `GET /reflection/eth-state/covers` before broadcasting. Building the reveal by hand (`tools/build-crossout-mint.mjs`)? Check that same endpoint yourself first. See [`BUILD-A-TACIT-DAPP.md` §5f](./BUILD-A-TACIT-DAPP.md#5f-bridging-between-chains). |
