...
export function makeConfidentialLp({ keccak256, pool }) {
  ...
  // The blindings rA/rB/rShares + nonces stay CLIENT-SIDE; only the opening sigmas reach the witness.
  function buildAdd({ assetA, assetB, chainBinding, reserveAPre, reserveBPre, sharesPre, aNote, bNote, dA, dB, rA, rB, shareOwner, rShares, nonceA, nonceB, nonceShares }) {
    ...
    const ctx = addCtx(op);
    op.aSig = openingSigma(op.dA, rA, ctx, nonceA);
    op.bSig = openingSigma(op.dB, rB, ctx, nonceB);
    op.sSig = openingSigma(op.dShares, rShares, ctx, nonceShares);
    return op;
  }
  ...
  function verifyAdd(op, { merkleRootFrom, spendRoot }) {
    ...
    const pid = poolId(op.assetA, op.assetB), lpAsset = lpShareId(pid);
  ...
  function buildRemove({ assetA, assetB, chainBinding, reserveAPre, reserveBPre, sharesPre, shareNote, dShares, rShares, aOwner, rA, bOwner, rB, nonceShares, nonceA, nonceB }) {
