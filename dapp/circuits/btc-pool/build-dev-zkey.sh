#!/usr/bin/env bash
# DEV-ONLY phase 2 for spend.circom. SIGNET ONLY. NOT FOR MAINNET.
#
# One contributor with local entropy plus a fixed dev beacon: whoever runs this script can forge proofs
# under the resulting key. Mainnet keys come from the multi-party ceremony
# (ops/research-btc-pool-client-proving.md, "Ceremony plan").
#
...
# Outputs (gitignored): build/spend_dev_signet_final.zkey, build/spend_dev_signet_vk.json
set -euo pipefail
cd "$(dirname "$0")"
...
[ -f build/spend.r1cs ] || bash build.sh
...
Z0=build/spend_dev_0000.zkey
Z1=build/spend_dev_0001.zkey
ZF=build/spend_dev_signet_final.zkey
VK=build/spend_dev_signet_vk.json
BEACON=deadbeefcafef00d0123456789abcdefdeadbeefcafef00d0123456789abcdef

$SNARKJS groth16 setup build/spend.r1cs "$PTAU" "$Z0"
$SNARKJS zkey contribute "$Z0" "$Z1" --name="btc-pool-dev-signet-NOT-FOR-MAINNET" \
...
echo "DEV zkey (signet only): $ZF  $(stat -f %z "$ZF" 2>/dev/null || stat -c %s "$ZF") bytes"
