// Proof system for dapp/circuits/evm-pool/transact.circom behind one interface — the envelope layout and the
// contract never name it:
//
//   system.prove(input, { onProgress }) → { wire, publicSignals }     input: buildWitness(...).input
//   system.verify(publics, wire)        → bool                        publics: 11 decimal strings
//
// Groth16 (BN254). Wire and vk-pin layout are identical to ../btc-pool-zk-prover.js (A‖B‖C, big-endian
// 32-byte limbs, G2 in snarkjs order); the Solidity verifier reads the same shape via the standard
// abi.encodePacked(uint256[8]) calldata a snarkjs-exported verifier expects.

import { sha256 } from './vendor/tacit-deps.min.js';
import { publicSignals as toPublicSignals } from './evm-pool-zk.js';

export const N_PUBLIC = 11;
...
// input: the object from makeEvmPoolZk().buildWitness(...).input
export async function proveTransact(input, { wasm, zkey, snarkjs, singleThread = false } = {}) {
  ...
  if (publicSignals.length !== N_PUBLIC) throw new Error('evm-pool-zk: unexpected public signal count');
  return { proof, publicSignals, wire: encodeProof(proof) };
}

// publics: array of decimal strings (see evm-pool-zk.js publicSignals) or the named object.
export async function verifyTransact(vk, publics, proof, { snarkjs } = {}) {
  const s = await loadSnarkjs(snarkjs);
  const signals = Array.isArray(publics) ? publics.map((x) => BigInt(x).toString()) : toPublicSignals(publics);
  if (signals.length !== N_PUBLIC) return false;
...
const be32 = (dec) => {
  ...
  if (hex.length > 64) throw new Error('evm-pool-zk: field overflow');
...
// SHA-256 over alpha1 ‖ beta2 ‖ gamma2 ‖ delta2 ‖ IC as big-endian limbs (G2 in snarkjs order).
export function vkHash(vk) {
  if (vk?.protocol !== 'groth16' || vk?.curve !== 'bn128' || Number(vk?.nPublic) !== N_PUBLIC) throw new Error('evm-pool-zk: not a transact.circom Groth16 key');
...
export function makeGroth16System({ vk, wasm = null, zkey = null, snarkjs = null, singleThread = false, pinnedVkHash = null } = {}) {
  if (!vk) throw new Error('evm-pool-zk: verification key required');
  const hash = vkHash(vk);
  if (pinnedVkHash && hash !== String(pinnedVkHash).replace(/^0x/, '').toLowerCase()) throw new Error(`evm-pool-zk: verification key ${hash} is not the pinned ${pinnedVkHash}`);
  const load = async (x, name) => {
    const v = typeof x === 'function' ? await x() : x;
    if (!v) throw new Error(`evm-pool-zk: ${name} unavailable`);
    return v;
  };
  return {
    ...
    async prove(input, { onProgress } = {}) {
      ...
      onProgress?.('proving');
      const { wire, publicSignals } = await proveTransact(input, { wasm: w, zkey: z, snarkjs: await loadSnarkjs(snarkjs), singleThread });
      if (!(await verifyTransact(vk, publicSignals, wire, { snarkjs: await loadSnarkjs(snarkjs) }))) throw new Error('evm-pool-zk: fresh proof does not verify');
    ...
    async verify(publics, wire) {
      ...
      return verifyTransact(vk, publics, b, { snarkjs: await loadSnarkjs(snarkjs) });
    },
  };
}
