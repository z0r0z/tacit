...
| F-6 | **High** (attester halt, JS mirror only) | **The JS mirror refunded every zero-tip swap batch the guest clears.** The reflection mirror's tip-opening check (added this morning) calls the prover's Pedersen `commit`, which multiplies the generators by the scalars as given; the secp library rejects a zero scalar, so a zero tip amount — the common case — threw, the check read as failed, and the mirror took the refund path while the guest cleared the batch. The attester's digest would diverge from the guest's on the first zero-tip batch and reflection would halt until patched (the drift class this round has been hunting). Surfaced by the readiness gate: `confidential-swapbatch-fold` failed its accept case. | **Fixed.** `commit` now treats a zero scalar as the identity, matching the guest's `verify_pedersen_opening`; every caller (fee-claim opening, note commitments) inherits it. The batch test now carries the tip openings and expects the guest's refund (not skip) for a settler-malformed receipt; the LP-remove fold test passes the share asset the mirror now filters on. Gate node suites all green. |
