...
- **Mechanical evidence:** the full Solidity suite (88 suites, 879 tests, 0 failures, on the fixed tree), the
  ...
  pin byte-for-byte; the pool's runtime was re-measured and re-pinned (23,720 B after this pass, 23,747 B — 829 B
  under EIP-170 — after the follow-up fix); the storage layout was re-read and no guest-pinned slot moved.
...
| F-3 | Design (next migration) | **Canonical-token continuity across generations.** `CanonicalBridgedERC20.MINTER` is immutable and in the CREATE2 salt, so every generation mints a distinct tacBTC / tacUSD / TAC; a retired generation bars cross-outs, so a public holder of a retired generation's token can re-enter that generation (after F-1) but has no path to Bitcoin or the successor. | **Recorded.** Recommended before the first `createNextGen`: lineage-aware mint authority in the canonical token (accept any pool reachable from `MINTER` through `successor()`), with the successor's constructor adopting its predecessor's tokens rather than deploying fresh ones — one public tacBTC across generations, no guest change. This deploy starts a fresh lineage, so nothing here gates it. |
| F-4 | Low (migration liveness) | The successor's first attest binds the predecessor's live digest and attest is permissionless, so a predecessor attest landing between the successor's proof generation and submission forces a rebuild (`StaleReflectionDigest`), at most once per matured Bitcoin block. Counters are frozen at retirement, so only the digest moves. | **Recorded.** If it ever bites, `createNextGen` can snapshot the handoff digest and tip; the successor re-folds anything folded after the snapshot. |
