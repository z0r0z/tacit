...
generations as a successor-side, per-migration escrow adoption rather than lineage minting (runbook), trimmed
the pool's dead interface declarations, and — from the readiness gate — caught and fixed a **High** JS-mirror desync: the
attester's tip-opening check threw on a zero tip (the secp library rejects a zero scalar), so every zero-tip swap
batch the guest clears would have been refunded by the mirror and halted reflection on first contact. Evidence: forge green on the final tree bar one regenerated fixture that lost
...
each (recorded in the report so they are not re-derived). Four hardening items and one defense-in-depth check were found off-chain and landed:
...
non-constant time; plus a redundant IPFS sub-path check (the URL parser already normalises dot segments, so it was not exploitable). A full git-history scan for publication
...
| Public release (post-deploy) | CLEAR TO PUBLISH — 0 Critical/High/Medium in the immutable surface; live mainnet bytecode proven byte-identical to source (link refs + immutables normalised), both vkeys and `CHAIN_BINDING` verified out of the deployed bytecode, all pin scripts green, forge 884/884; 10 attacks constructed and refuted; 4 off-chain hardening fixes (relay submit metering, prover fixture path, CDP CSPRNG requirement, constant-time bearer gate) + 1 defense-in-depth check; git-history secret scan clean @ `f3917087` | Claude Opus 5 | `AUDIT-2026-09-19-public-release-review` |
...
| V1 launch (post-deploy) | GREEN-LIT FOR LAUNCH — 0 findings in the immutable surface; forge 884/884, all five pin gates green. Verified end-to-end (not assumed) that the cold-start cross-out brick is CLOSED on gen5: the `count==0` exclusion proof is in the eth-reflection guest, its recursion digest is pinned into `reflect.rs`, and that chain produced the live pool's immutable `BITCOIN_RELAY_VKEY` — so the first public ETH->BTC bridge cannot freeze the lane. Live state read during the review shows the counter already seeded (`attestedBitcoinConsumedCount == 1`, `crossOutCount == 0`, `successor == 0`), so the old operational precondition is satisfied too. 4 launch items, all off-chain: an unauthenticated request-body DoS on the public API (the Cloudflare body ceiling had no Node equivalent after the Render migration; one large POST recycles the process — FIXED here, needs an API redeploy), and three of five live token-list entries still pointing at the retired gen4 suite (needs a multisig refresh), plus a fast-lane liveness gap: nothing registers a consumed source, the sidecar never publishes one and the only client for the endpoint that would is never called, so `buildModeBBatch` throws and the next fast-lane spend stalls Mode-B until an operator posts it by hand (recoverable, no funds at risk); and the wallet scanner has no `T_CXFER_BOUND` (0x39) branch, so generation-bound Bitcoin notes — exactly the fast-lane-spendable class — are never surfaced (pre-existing, caught by the repo's own recovery-parity guard test). JS suite characterized: 173 pass / 17 fail / 9 network-blocked, all 17 failures reproduced at the base commit. Stale gen3/gen4 addresses purged from the published manifests and integration guides. Recorded, not changed: relayed swaps reveal their amounts to the relay operator because `OP_SWAP_BLIND` is armed in the guest but wired into neither the API allowlist nor the relay op map | Claude Opus 5 | `AUDIT-2026-09-20-v1-launch-review` |
