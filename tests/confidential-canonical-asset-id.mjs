...
export function metaHash(symbol, decimals) {
  const s = Buffer.from(symbol, 'utf8');
  if (s.length > 255) throw new Error('label too long');
  return sha256(Buffer.concat([u8(s.length), s, u8(decimals)]));
}

export function deriveAssetId({ chainId, factory, salt, etcher, symbol, decimals }) {
  ...
  return sha256(Buffer.concat([
    tag, chainBe8, addr(factory), b32(salt), addr(etcher), metaHash(symbol, decimals),
  ]));
}
...
assert.strictEqual(hex(metaHash('TAC', 18)),
  '0xdf3026173e81ffe48ad033a90d78054b461ea8303f5d76989bd8d5e050311215', 'metaHash KAT');
ok('metaHash(TAC,18) matches the Solidity KAT (cross-language)');

// 2. metadata is bound: any change to symbol/decimals changes the id
...
ok('asset id changes if symbol or decimals changes (the per-asset metadata is bound)');
