...
//
// Wire format (157 bytes total):
//     A_secp (33)  || A_BJJ (32) || z_a (28) || z_r_secp (32) || z_r_BJJ (32)
//
// Parameters (AMM.md §3.10):
//     e < 2^80   (challenge — Fiat-Shamir from SHA256)
//     α < 2^224 - 2^144   (mask — rejection-sampled so z_a < 2^224 fits in 28 bytes)
//     z_a = α + e·a       (over the integers)
//     z_r_secp = β_secp + e·r_secp   (mod n_secp)
//     z_r_BJJ  = β_BJJ  + e·r_BJJ    (mod n_BJJ)
//
// Soundness: 80-bit (Fiat-Shamir extractor with challenge space 2^80).
// Statistical ZK: 80-bit margin on `a`.
//
// Verifier reduces `z_a` into each group's scalar field. Since z_a < 2^224 and
// n_secp ≈ 2^256, n_BJJ ≈ 2^251, both reductions are identity — the same integer
// is used in both equations, which is what binds `a` across groups.
...
const TWO_80  = 1n << 80n;
const TWO_144 = 1n << 144n;
const TWO_224 = 1n << 224n;
const ALPHA_MAX = TWO_224 - TWO_144; // α uniform in [0, 2^224 - 2^144)
...
// Uniform random sample in [0, max) by rejection from 28 random bytes.
...
// Compute the Fiat-Shamir challenge e < 2^80 from the public transcript.
//   e = bytes_be_to_int( SHA256(domain || C_secp || C_BJJ || A_secp || A_BJJ) )[-10:] mod 2^80
// We take the LOW 10 bytes of the digest (i.e., `digest mod 2^80` when the
...
export function challenge(C_secp_bytes, C_BJJ_bytes, A_secp_bytes, A_BJJ_bytes) {
  ...
  return bytesToBigintBE(h.subarray(22, 32)); // last 10 bytes = 80 bits
}
...
//
// Returns:
//   { proof: Uint8Array(157), C_secp_bytes, C_BJJ_bytes }
export function proveXCurve({ a, r_secp, r_BJJ, C_secp = null, C_BJJ = null, rng = defaultRng }) {
  ...
  const z_a = alpha + e * a;
  if (z_a >= TWO_224) {
    // Statistically impossible (α ≤ 2^224 − 2^144, e·a ≤ 2^144 − 1 ⇒ z_a < 2^224).
    // Keep as defensive guard.
    throw new Error('z_a overflowed 28 bytes (resample required)');
  }
  ...
  const proof = concatBytes(
    A_secp_bytes,                       // 33
    A_BJJ_bytes,                        // 32
    bigintToBytesBE(z_a, 28),           // 28
    bigintToBytesBE(z_r_secp, 32),      // 32
    bigintToBytesBE(z_r_BJJ, 32),       // 32
  );
  if (proof.length !== 157) throw new Error(`proof len ${proof.length}, expected 157`);
...
export function verifyXCurve(proof, C_secp_bytes, C_BJJ_bytes) {
  if (!(proof instanceof Uint8Array) || proof.length !== 157) return false;
  ...
  const z_a          = bytesToBigintBE(proof.subarray(65, 93));
  const z_r_secp     = bytesToBigintBE(proof.subarray(93, 125));
  const z_r_BJJ      = bytesToBigintBE(proof.subarray(125, 157));

  // Range checks
  if (z_a >= TWO_224)   return false;          // 28-byte BE encoding bound
  ...
  // z_a is < 2^224 ≪ n_secp, so reduction is identity.
  ...
  // z_a is < 2^224 < n_BJJ, reduction is identity.
  const lhsB = addPoint(
