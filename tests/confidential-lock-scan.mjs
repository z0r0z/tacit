...
// ── 4. Real mainnet fixture: the actual settle() calldata of an OP_STEALTH_LOCK on the live pool.
// Hardcoded raw, not built by this file's own encoder. The contract does
// `abi.decode(publicValues, (PublicValues))`, and because PublicValues contains dynamic fields, ABI
// encodes it as a single-element tuple — prepending an extra offset word (0x20) before the struct's
// own fields. decodePublicValuesLockFields must read past that offset word rather than treating it as
// the struct's first field. A synthetic fixture built by this file's own encoder can't catch a decoder
// and encoder sharing the same wrong assumption about that layout, so this real fixture is the
// independent check.
{
