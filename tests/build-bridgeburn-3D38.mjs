...
const OWNER = '0x' + '11'.repeat(32); // pins the pre-existing on-chain leaf0/leaf1 (KNOWN_LEAF0/1)
const BTC_OWNER = '0x' + Buffer.from('btc-dest-owner'.padEnd(32, '\0')).toString('hex');
// EVM notes are bearer: the burn input's spend owner is H(nk), and native_nu binds ν to the secret nk.
...
const IN_OWNER = pool.nkToOwner(IN_NK); // the burn input note's H(nk) owner
...
const leaf2 = pool.leaf(ASSET, c2.cx, c2.cy, IN_OWNER);
const wrapOp2 = { chainBinding: CHAIN_BINDING, asset: ASSET, value: V2.toString(), cx: c2.cx, cy: c2.cy, owner: IN_OWNER, blinding: be32(r2) };
const depositId2 = pool.depositId(ASSET, V2, c2.cx, c2.cy, IN_OWNER);
...
const bindNu = pool.nativeNullifier(IN_NK, leaf2);
...
const bridgeBurnOp = {
  ...
  inputs: [{ cx: c2.cx, cy: c2.cy, owner: IN_OWNER, leafIndex: 2, path: path2, nk: IN_NK }],
