...
slash its escrow. The amount minted never depends on an oracle; a price
feed only sizes the escrow.
...
`OP_CHECKSIGFROMSTACK`, `OP_VAULT` or an equivalent, three constructions Tacit reserves room for
become trustless:
cBTC locks whose sats can leave only through a redemption, bids escrowed on-chain instead of by a
watchtower, and BTC split into
fungible shares with no shared vault.
...
receiver, and every node checks it from Bitcoin alone. cBTC is its path
