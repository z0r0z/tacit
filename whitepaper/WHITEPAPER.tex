...
swap, add liquidity or borrow (\S6). TAC, cBTC and cUSD have ERC-20s that only the pool mints:
...
\textbf{Trades without a pool.} \op{OP\_OTC} swaps two parties' notes directly. \op{OP\_BID} is a limit order that a
...
Bitcoin cannot yet restrict where an output may be spent. Once it can, through CTV, \op{OP\_CAT}, \op{OP\_CHECKSIGFROMSTACK}, \op{OP\_VAULT} or an equivalent, three
