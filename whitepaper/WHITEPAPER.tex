...
specification and its open reference implementations. The Bitcoin reflection guest (\S5) is a second,
...
\item \textbf{Trades:} atomic trades against BTC, bids that fill while the buyer is offline, and a native AMM
...
\textbf{Entry and exit.} \op{wrap} escrows ETH or any ERC-20 and records a deposit that only its depositor's
...
\textbf{Settlement.} Every change to notes, locks and positions is a batch of up to 256 ops proven by one SP1
...
\item \textbf{Fast lane.} A Bitcoin note bound to this deployment is spent directly on Ethereum. Its proof shows
...
\begin{itemize}
\item re-mints on Bitcoin of notes crossed out of the pool;
...
\textbf{Swaps.} Pools are constant-product curves with a fee tier and an optional protocol fee. Reserves are
public. A plaintext AMM contract, \op{TacitPublicAmm}, trades against the same reserves, so public and
confidential liquidity share one curve.

\begin{itemize}
\item \op{OP\_SWAP\_ROUTE} swaps a hidden input note through up to four pools, with an end-to-end minimum output.
\item \op{OP\_SWAP} clears many hidden-output intents at one uniform price. Traders in one batch pay the same
  price, so they cannot sandwich each other, and the batch reveals only its net change to reserves.
\end{itemize}

\textbf{Prover-blind swaps.} Whoever proves an ordinary swap sees its amounts. \op{OP\_SWAP\_BLIND} keeps them out
...
carries one Groth16 proof of the \op{amm\_swap\_batch} circuit, which proves over hidden amounts that the
...
through \op{OP\_SWAP} and \op{OP\_SWAP\_ROUTE} until the relay's batch coordinator ships.
...
for \op{T\_SWAP\_BATCH} on Bitcoin.
...
\textbf{Stealth payments.} A payer locks a note under a one-time key derived from the recipient's published
...
\textbf{cBTC.} A user locks BTC on Bitcoin with a \op{T\_CBTC\_LOCK} envelope that pre-commits to the note it will
mint. Reflection records the lock's value, and the pool mints cBTC once per lock. The locker keeps the
lock's key. Anyone may fund a wstETH escrow of at least 1.5$\times$ the lock's value, which makes spending the
...
a pilot. Its custody is economic until covenants make it enforced (\S11).
...
\textbf{cUSD.} A CDP locks a basket of cBTC notes and mints cUSD. The position's owner is hidden; its amounts
...
\op{CollateralEngine} that the pool only ever asks to accept or reject, with on-chain caps on its ratios and
fee, and a cooling-off period before a feed change or a stricter liquidation ratio takes effect.

\textbf{Farms.} LP-share notes bond into a receipt note. A farm controller tracks reward-per-share at entry,
...
\textbf{Self-sovereign.} The relayer is optional. \op{settle} is open to anyone, proofs can be generated locally
...
successor through \op{createNextGen}, its one privileged call, which the ops multisig holds. The call runs
...
witness on one device end to end, and \op{OP\_SWAP\_BLIND} keeps trade sizes from the SP1 prover once its coordinator ships.
...
\item for a walk-away bid on Bitcoin only, a hosted watchtower holding a capped, dedicated key over the sats
  the buyer commits.
\end{itemize}
...
\item cBTC is economically secured until covenants.
...
\item \textbf{Covenant cBTC.} A lock whose sats can leave only through a redemption. Pool op 5,
  \op{OP\_COVENANT\_MINT}, is held for its mint, which reuses the existing value binding with no escrow.
\item \textbf{On-chain bid escrow.} A buyer's pre-signed input bound to its bid template, replacing the
...
\textbf{V2.} V1 is built to hand off to this covenant generation. Its Bitcoin opcodes arrive additively, and
its settle logic uses the op codes V1 keeps free. V2 deploys through V1's own \op{createNextGen} and
inherits the Bitcoin lane (\S8). cBTC then moves from economic to enforced custody one lock at a time: a
...
\textbf{A shielded pool on Bitcoin.} Bitcoin opcodes \op{0x6C} and \op{0x6D} are reserved for a shielded pool that
lives on Bitcoin alone, described in the companion paper \emph{Secret Sats}. It holds Tacit assets, not BTC,
