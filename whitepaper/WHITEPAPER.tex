...
\textbf{Cross-chain calls.} Beside value, reflection carries authenticated calls in both directions, so each
...
\begin{itemize}
\item \textbf{Bitcoin to Ethereum.} A \op{T\_BTC\_\allowbreak{}CALL} envelope is a Bitcoin key's Schnorr-signed authorization of one
  Ethereum call: a target contract and its calldata. The reflection guest checks the signature, and the
  pool records the call in its inbox of pending calls. \op{BtcCallExecutor}, a separate contract with no
...
\item \textbf{Ethereum to Bitcoin.} Any Ethereum contract can post a message to \op{EthCallOutbox} under a handler
  namespace. The light-client proof carries it to Bitcoin, where a \op{T\_ETH\_\allowbreak{}CALL} envelope delivers it into
...
\item a bridge made of proofs in both directions, for value and for calls;
