...
\begin{abstract}
\noindent
Bitcoin holds value without a trusted party, but issuing assets on it, hiding amounts or
...
\begin{center}
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{0.64\linewidth}@{}}
\toprule
...
\begin{itemize}
\item \op{OP\_SWAP\_ROUTE} swaps a hidden note through up to four pools, with an end-to-end minimum output.
...
\textbf{Prover-blind swaps.} Whoever proves an ordinary swap sees its amounts. \op{OP\_SWAP\_BLIND} hides them from
...
\op{amm\_swap\_batch} circuit shows, over hidden amounts, that the price, every fill and every minimum output
...
verifying keys fix it. The same circuit clears \op{T\_SWAP\_BATCH} on Bitcoin.
...
\textbf{cBTC.} A user locks BTC on Bitcoin with a \op{T\_CBTC\_LOCK} envelope that names the note it will mint.
...
\midrule
Bitcoin & amounts; stealth recipients; trade sizes in \op{T\_SWAP\_BATCH} & addresses, the transaction graph, asset ids, burn amounts, AMM reserves \\
...
keeps a witness on one device, and \op{OP\_SWAP\_BLIND} keeps trade sizes from the prover too.
...
\item \textbf{Covenant cBTC.} A lock whose sats can leave only through a redemption. Pool op 5,
  \op{OP\_COVENANT\_MINT}, is held for its mint, which needs no escrow.
