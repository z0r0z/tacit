# Tacit V1 — authoritative security audit

You are performing a **full, independent, adversarial security audit** of the **Tacit V1 immutable core**: a
...
Your review is intended to be published as an authoritative audit. Depositors, bridgers, LPs, and integrators
will rely on it. Treat completeness as a first-class obligation: it is better to explicitly bound your coverage
than to imply a soundness you did not establish.
...
the source under `guest/` and `contracts/`.

`CHANGES-SINCE-LAST-ROUND.md` records every fix applied since the previous review round — read it to focus effort, but audit the code independently.

**Scope note:** `BitcoinLightRelay.sol` (`contracts/src/lib/`) is now IN scope — the reflection anchor and every Bitcoin-derived property depend on it. The legacy denomination-pool mixer (`TacitBridgeMixer`, its `SP1PoolRootVerifier` guest, and the standalone 5-signal `Groth16Verifier`) is NOT part of the Tacit V1 confidential surface and is out of scope.
...
- **The confidential note model & accumulators** — Pedersen commitments, the bearer/kernel spend model, leaf &
  nullifier derivation, the note tree / spent-set IMT / UTXO IMT / bridge-burn set / lock set, range proofs,
  and every cryptographic primitive (`groth16.rs`, `babyjubjub.rs`, `bjj.rs`, `sigma.rs`, the circom circuits).
...
- **The two-way bridge & reflection** — the reflection guest's per-op folds, the Bitcoin tx/relay parsing
  ...
  Mode-B reverse reflection, the provenance DAG for scan-free onboarding, and cross-generation/resume.
- **All contracts** — `ConfidentialPool`, `ConfidentialRouter`/`ExitExecutor`, `CollateralEngine`,
  `FarmController`, the canonical asset/minter/bridged-ERC20 factory, `BitcoinLightRelay` (the Bitcoin light
  client the reflection anchor trusts — in scope), `TacitRelayer`, `BtcCallExecutor`, `ChainlinkEthBtcAdapter`.
...
- The `amm_swap_batch` circuit and its ceremony key are **locked**; you may audit the circuit's constraints and
  the in-guest verifier for soundness, but treat the ceremony/vkey as fixed (a circuit change is future work).
- **Bytecode/vkey reproducibility and guest↔dapp mirror parity are a separate build step** — this bundle is
  source. If a property depends on the compiled ELF/vkey or the off-chain mirror matching the source, say so;
...
- An honest **coverage ledger and residual-risk read**: what you examined and how deeply, what you did not
  reach, and your assessment of the remaining known- and unknown-unknowns — so a reader knows exactly what this
  audit does and does not certify.

If it clears the bar, say so unambiguously and explain the basis. If it does not, give the single most important
thing to fix.
