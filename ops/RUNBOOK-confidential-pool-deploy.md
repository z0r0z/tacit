...
> The first reflection batch is LOCKED to start at `GENESIS_REFLECTION_ANCHOR` (its `headers[0].prev`). Its tip must be at or below `RELAY.tip() − REFLECTION_CONFIRMATIONS`, and may sit up to `REFLECTION_MAX_LAG` (2016) further below — so a gap does **not** have to be closed by one batch. **Still set the genesis anchor at the live matured relay tip and attest continuously**: 1–6 block batches prove in ~2 min on the standard RTX 4090 box, and staying near the tip keeps the pool's anchor walk (one `blockParent` read per block of lag) cheap. If the anchor does sit far below the relay tip, work the gap off as a SEQUENCE of ordinary batches — each proves, lands, and advances `lastReflectionBlockHash` before the next is assembled. Do not attempt to span a large gap in one full-scan: a 73-block gap on a busy signet is ~15k txs and **OOMs a 24 GB GPU** (it's the recursion circuit, not shard size). Only a gap larger than `REFLECTION_MAX_LAG` forces a single batch big enough to land back at the matured anchor.
