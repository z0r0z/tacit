...
> **Status note (2026-06-21):** this is a historical readiness/gate narrative. For the current Sepolia
> full-suite bring-up, use `ops/STATUS-confidential-system.md`,
> `ops/RUNBOOK-confidential-pool-deploy.md`, `ops/CHECKLIST-sepolia-full-suite.md`, and the
> authoritative top-level pins in `contracts/sp1/confidential/elf-vkey-pin.json`.
...
**POOL**
- **vkey coherence — enforced.** Gate layer 4 cross-checks the pin (`elf-vkey-pin.json program_vkey`) == deploy `DEFAULT_VKEY` == every settle fixture, and `DeployConfidentialPool.s.sol` reverts a deploy whose `PROGRAM_VKEY` != the pin (override only via `ALLOW_UNPINNED_VKEY=1`). As of the Sepolia full-suite cleanup, the authoritative deploy vkeys are the top-level `program_vkey` and `bitcoin_relay_vkey` fields in `contracts/sp1/confidential/elf-vkey-pin.json`; run `contracts/sp1/confidential/verify-vkey-pin.sh` before any deploy.
...
- **Swap/LP output binding — CLOSED (opening sigma, Parity F1).** The OP_SWAP/OP_LP witness binds amounts with an *opening sigma* — a Schnorr proof of knowledge of the note blinding for the public amount, whose challenge binds the whole intent (`out_owner`/`min_out`/reserves) — instead of exposing the raw blinding `r`. So the settle prover verifies the openings WITHOUT learning `r` and can neither spend the input nor redirect the output (the swap/LP analog of the kernel hiding transfer blindings). ProofReal fixtures must match the current pinned vkey; client parity lives in `tests/confidential-{swap,lp}-op.mjs` + `confidential-opening-sigma.mjs`.
...
- **Bridge cross-lane settle proof (L5) — DONE.** A real settle proof exercising the cross-lane non-membership gate verifies on-chain (`ConfidentialCrossLaneProofReal`, fixture `crosslane_groth16.json`); fixture vkey must match the current pin.
