...
| burn-deposit.json | burn-deposit 0x2B | scan-free onboarding of a never-reflected note proven real via provenance | `gen-reflection-burn-deposit` | DIGEST_MATCH — recorded under `bridge_burn_id(DEPOSIT, burn_outpoint, native_leaf)`; spent + burn folded independently | 0x5435aa27… |
...
- **Burn-deposit** (`spends.is_empty()`, provenance-verified): key = `bridge_burn_id(BURN_SOURCE_DEPOSIT,
  ...
  appending the native note; a duplicate burn_id is a membership-gated no-op.
