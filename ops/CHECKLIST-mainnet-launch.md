...
- [ ] **Fast-lane FRESHNESS gate** (built in code, re-prove pending) — beyond the coherence gate above, completeness against the contract's *recorded* consume set: `ConfidentialPool.bitcoinConsumedCount` (slot 120) is the monotone count of consumed entries, and the eth-reflection guest reads it from the same finalized state and asserts `consumed_count == bitcoinConsumedCount` — so a worker cannot witness only a subset of consumes and leave the omitted source notes live + double-spendable on Bitcoin. Landed: contract counter + forge tests (`test_fast_lane_consumed_count_*`) + the guest assert. Box step: add slot 120 to the eth-reflection `eth_getProof` witness; the re-prove that rotates `ETH_REFLECTION_VKEY` folds this into the SAME atomic bundle as the coherence gate
