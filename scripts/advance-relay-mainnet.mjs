...
//   RELAY_ADDRESS=0x20A6ddc2C6E620c6248B5A34E85996516FDd19D0 RELAY_PK=0x... \
//   MEMPOOL_API=https://blockstream.info/api \
//   node scripts/advance-relay-mainnet.mjs --count 10
//
// Options:
...
async function mempool(path) {
  // node's fetch (undici) can't complete a TLS handshake to some esplora hosts in this sandbox even
  // though curl succeeds against the same host — shell out to curl instead of debugging undici.
  const { execFile } = await import('node:child_process');
  const url = `${MEMPOOL_API}${path}`;
  const out = await new Promise((resolve, reject) => {
    execFile('curl', ['-sf', '--max-time', '15', url], (err, stdout) => {
      if (err) reject(new Error(`${url}: ${err.message}`)); else resolve(stdout);
    });
  });
  return out.trim();
}
...
// This relay build (contracts/src/lib/BitcoinLightRelay.sol) derives each block's difficulty target
// from its own branch's blockTarget/epochStartTs per advanceTip call, with no separate on-chain
// currentEpoch()/boundary getter — it retargets internally and can cross a 2016-block epoch boundary
// within a single advanceTip call, so no boundary cap is needed here.
