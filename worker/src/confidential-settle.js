...
export function makeConfidentialSettler({ storage, hash, now, feeGate }) {
  ...
  async function submitJob({ type, op, memos, mode = 'settle', feeAsset = null }) {
    ...
    if (!['wrap', 'unwrap', 'transfer', 'swap', 'route', 'lp', 'otc', 'bid', 'bridgeburn', 'cdpmint', 'farmbond', 'farmharvest', 'farmunbond', 'adaptorlock', 'adaptorclaim', 'adaptorrefund', 'cdpclose', 'cdpliquidate', 'cdptopup', 'bridgemint', 'cbtcmint', 'stealthlock', 'stealthlockbatch', 'stealthclaim', 'stealthrefund', 'bridgestealthmint', 'wraptransfer', 'sendunwrap', 'lpbond', 'lpremove', 'batchtransfer', 'wrapcdpmint'].includes(type)) throw new Error(`submitJob: unknown type ${type}`);
