...
const T_CETCH    = 0x21;
const T_CXFER_BPP = 0x22; // BP+ variant of T_CXFER, identical wire shape (SPEC §5.47 amendment)
const T_CXFER    = 0x23;
const T_MINT     = 0x24;
const T_BURN     = 0x25;
...
// tests/amm-validator.mjs AMM_INITIAL_LP_LOCK_BLOCKS byte-for-byte.
const AMM_INITIAL_LP_LOCK_BLOCKS = 6;
// Solo-intent confidentiality default (POOL_CAP_SOLO_INTENT_ALLOWED bit).
// AMM.md normative: T_SWAP_BATCH with n_intents < AMM_MIN_BATCH_SIZE rejected
// unless pool opts in via capability_flags bit 1 (0x02). For T_SWAP_VAR the
// batch-size question doesn't apply (always 1 trader).
const AMM_MIN_BATCH_SIZE = 2;
const POOL_CAP_SOLO_INTENT_ALLOWED = 0x02;
// Protocol-fee cap (AMM.md §"Protocol fee mechanism"): basis points of LP-fee
// growth, max 10% (1000 bps).
const AMM_PROTOCOL_FEE_BPS_MAX = 1000;
...
// at different fee tiers / capability flags (V3/V4 parity per AMM.md §"One
...
// pool_id, including fee_bps + capability_flags).
function ammPairKey(network, lowHex, highHex) {
  return network === 'signet'
    ? `ammpoolpair:${lowHex}:${highHex}`
    : `ammpoolpair:${network}:${lowHex}:${highHex}`;
}
async function ammPairGet(env, network, lowHex, highHex) {
  const v = await env.REGISTRY_KV.get(ammPairKey(network, lowHex, highHex), 'json');
  return Array.isArray(v) ? v : [];
}
async function ammPairAppend(env, network, lowHex, highHex, poolIdHex) {
  const list = await ammPairGet(env, network, lowHex, highHex);
  if (list.includes(poolIdHex)) return list;
  list.push(poolIdHex);
  await env.REGISTRY_KV.put(ammPairKey(network, lowHex, highHex), JSON.stringify(list));
  return list;
}
...
// ============== Bulletproof verifier (range-proof aggregate, m ∈ {1,2,4,8}) ==============
//
// Mirrors tests/bulletproofs.mjs `bpRangeAggBatchVerify` byte-for-byte —
// must match the dapp's prover transcript labels + domain so T_SWAP_VAR
// envelopes generated by the dapp verify against this worker code.
//
// Generators are derived lazily on first use via try-and-increment in the
// `tacit-bp-{G,H,Q}-v1` domains (BP_MAX_M=8 × N_BITS=64 = 512 G + 512 H +
// 1 Q points). Generator derivation is ~500ms one-time per worker isolate;
// subsequent verifies are msm + sha256 (microseconds in JS).
//
// Critical: T_SWAP_VAR's m=2 bulletproof over (C_change_or_sentinel,
// C_receipt_secp) is the change-side inflation defense. Without verification
// a trader can claim delta_in_total > a_in_real, mint C_change to commit to
// (a_real - delta_in_total) mod n (which wraps to a huge positive value
// under BigInt), and inflate asset A by Δ when spending the change UTXO.
const _BP_N_BITS = 64;
const _BP_MAX_M = 8;
const _BP_MAX_NM = _BP_N_BITS * _BP_MAX_M;
...
function _bpHashToCurve(domain, idx) {
  ...
  for (let counter = 0; counter < 256; counter++) {
    ...
    try {
      const p = secp.ProjectivePoint.fromHex(bytesToHex(candidate));
      if (!p.equals(PEDERSEN_ZERO)) return p;
    } catch {}
  }
...
function _vecOnes(n) { return new Array(n).fill(1n); }
...
function _bpModInv(a) {
  let [oldR, r] = [modN(a), SECP_N];
  let [oldS, s] = [1n, 0n];
  while (r !== 0n) {
    const q = oldR / r;
    [oldR, r] = [r, oldR - q * r];
...
function _bpMsm(scalars, points) {
  // Simple windowed MSM. Naive sum is fine for moderate sizes (worker
  // verifies once per swap; cost is sub-100ms in JS).
  let acc = PEDERSEN_ZERO;
...
function _bpSafeMult(P, s) {
  const x = modN(s);
  return x === 0n ? PEDERSEN_ZERO : P.multiply(x);
}
function _bpTranscript() {
  const parts = [];
...
function _bpRandomScalar() {
  while (true) {
    const buf = crypto.getRandomValues(new Uint8Array(32));
    const x = bytes32ToBigint(buf);
    if (x !== 0n && x < SECP_N) return x;
  }
}
function _bpBigintToBytes32(n) {
  const m = modN(n);
  const hex = m.toString(16).padStart(64, '0');
  return hexToBytes(hex);
}
// Aggregated range-proof verifier. items[i] = { commitments: [V_pts...],
// proof: Uint8Array }. m ∈ {1,2,4,8}; n_bits = 64.
function bpRangeAggBatchVerify(items, n_bits = _BP_N_BITS) {
  ...
  for (let pIdx = 0; pIdx < items.length; pIdx++) {
    ...
    let A, S, T_1, T_2;
    try {
      A   = compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33))); off += 33;
      S   = compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33))); off += 33;
      T_1 = compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33))); off += 33;
      T_2 = compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33))); off += 33;
    } catch { return false; }
    ...
    try {
      for (let k = 0; k < log_nm; k++) {
        Lk.push(compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33)))); off += 33;
        Rk.push(compressedPointFromHex(bytesToHex(proofBytes.slice(off, off + 33)))); off += 33;
      }
    } catch { return false; }
    ...
    const transcript = _bpTranscript();
    ...
    for (const V of V_pts) transcript.append('V', V.toRawBytes(true));
    transcript.append('A', A.toRawBytes(true));
    transcript.append('S', S.toRawBytes(true));
    const y = transcript.challenge('y');
    const z = transcript.challenge('z');
    transcript.append('T1', T_1.toRawBytes(true));
    transcript.append('T2', T_2.toRawBytes(true));
    const x = transcript.challenge('x');
    transcript.append('t_hat', _bpBigintToBytes32(t_hat));
    transcript.append('tau_x', _bpBigintToBytes32(tau_x));
    transcript.append('mu', _bpBigintToBytes32(mu));
    const w = transcript.challenge('w');
    const u = new Array(log_nm);
    for (let j = 0; j < log_nm; j++) {
      transcript.append('L', Lk[j].toRawBytes(true));
      transcript.append('R', Rk[j].toRawBytes(true));
      u[j] = transcript.challenge('u');
    }
    const u_inv = _bpBatchInv(u);
    ...
    for (let i = 1; i < nm; i++) {
      ...
      const jj = log_nm - 1 - j_lsb;
      s[i] = modN(s[i ^ lsb] * u_sq[jj]);
    }
    const s_inv = _bpBatchInv(s);

    const ones_nm = _vecOnes(nm);
    const y_nm = _vecPow(y, nm);
    const sum_y_nm = _vecInner(ones_nm, y_nm);
    ...
    const y_inv = _bpModInv(y);
    const y_inv_pow = _vecPow(y_inv, nm);
    const zpow_2j = new Array(m); { let p = z_sq; for (let j = 0; j < m; j++) { zpow_2j[j] = p; p = modN(p * z); } }
    const two_n = _vecPow(2n, n_bits);

    const alpha = _bpRandomScalar();
    const beta  = _bpRandomScalar();
    const x2 = modN(x * x);
  ...
  const allPoints  = [...Gvec, ...Hvec, Q, PEDERSEN_G, PEDERSEN_H, ...extraPoints];
  return _bpMsm(allScalars, allPoints).equals(PEDERSEN_ZERO);
}
function bpRangeAggVerify(V_pts, proofBytes, n_bits = _BP_N_BITS) {
  return bpRangeAggBatchVerify([{ commitments: V_pts, proof: proofBytes }], n_bits);
}

// ============== JCS canonicalization + launcher-pubkey extraction ==============
//
// Mirrors tests/amm-jcs.mjs byte-for-byte. RFC 8785 JSON Canonicalization
// restricted to tacit's value shapes (objects, arrays, strings, finite
// integer numbers, booleans, null). The launcher gate per AMM.md §"Optional
// launcher gate" REQUIRES the fetched metadata blob to be byte-identical
// to its JCS-canonical form — non-canonical blobs conservatively default
// to "no gate" rather than failing POOL_INIT outright.
function _ammJcsSerializeNumber(n) {
  if (!Number.isFinite(n)) throw new Error('JCS: NaN/Infinity not allowed');
  if (!Number.isInteger(n)) throw new Error('JCS: non-integer numbers not supported');
  if (Math.abs(n) > Number.MAX_SAFE_INTEGER) throw new Error('JCS: integer exceeds 2^53');
  return String(n);
}
function _ammJcsSerializeString(s) {
  let out = '"';
...
function _ammJcsSerialize(v) {
  if (v === null) return 'null';
  if (typeof v === 'boolean') return v ? 'true' : 'false';
  if (typeof v === 'number') return _ammJcsSerializeNumber(v);
  if (typeof v === 'string') return _ammJcsSerializeString(v);
  if (Array.isArray(v)) {
    if (v.length === 0) return '[]';
    let out = '[';
    for (let i = 0; i < v.length; i++) {
      if (i > 0) out += ',';
      out += _ammJcsSerialize(v[i]);
    }
    return out + ']';
  }
  if (typeof v === 'object') {
    ...
    for (let i = 0; i < keys.length; i++) {
      if (i > 0) out += ',';
      out += _ammJcsSerializeString(keys[i]) + ':' + _ammJcsSerialize(v[keys[i]]);
    }
    return out + '}';
  }
  throw new Error(`JCS: unsupported type ${typeof v}`);
}
function ammJcsCanonicalize(value) {
  return new TextEncoder().encode(_ammJcsSerialize(value));
}
function ammBytesEqual(a, b) {
  if (a.length !== b.length) return false;
  for (let i = 0; i < a.length; i++) if (a[i] !== b[i]) return false;
  return true;
}
// Extract the optional launcher pubkey from a fetched metadata blob.
// Conservative defaults per AMM.md: malformed / non-canonical / missing
// field → no gate (return null). 33-byte compressed pubkey hex required.
function ammExtractLauncherPubkey(blobBytes) {
  if (!(blobBytes instanceof Uint8Array)) return null;
  let parsed;
  try { parsed = JSON.parse(new TextDecoder('utf-8').decode(blobBytes)); }
  catch { return null; }
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) return null;
  let canonical;
  try { canonical = ammJcsCanonicalize(parsed); }
  catch { return null; }
  if (!ammBytesEqual(canonical, blobBytes)) return null;
  const v = parsed.tacit_amm_launcher;
  if (typeof v !== 'string') return null;
  if (v.length !== 66 || !/^[0-9a-f]{66}$/.test(v)) return null;
...
//
// Per AMM.md §"MINIMUM_LIQUIDITY burn-output construction": at POOL_INIT,
// vout[1] MUST be a P2WPKH paying HASH160(0x02 || NUMS_recipient_x) where
// NUMS_recipient is derived deterministically from pool_id (try-and-
// increment x-only point), AND the envelope's Pedersen commit for that
// output MUST equal C_min_liq = MINIMUM_LIQUIDITY · H + r_burn · G with
// r_burn = SHA256("tacit-amm-min-liq-blind-v1" || pool_id) mod n_secp.
//
// Without this check, a founder can mint all isqrt(deltaA·deltaB) shares
// to themselves (including the nominal 1000-share lock) and later
// T_LP_REMOVE the entire pool — reopening the first-LP donation-inflation
// attack that MINIMUM_LIQUIDITY is supposed to close.
const _MIN_LIQ_DOMAIN_BLIND = new TextEncoder().encode('tacit-amm-min-liq-blind-v1');
const _MIN_LIQ_DOMAIN_PUBKEY = new TextEncoder().encode('tacit-amm-min-liq-pubkey-v1');
function ammDeriveMinLiqBlinding(poolIdBytes) {
  const seed = sha256(concatBytes(_MIN_LIQ_DOMAIN_BLIND, poolIdBytes));
  return modN(bytes32ToBigint(seed));
}
function ammDeriveMinLiqCommitment(poolIdBytes) {
  const r = ammDeriveMinLiqBlinding(poolIdBytes);
  return pedersenCommit(AMM_MINIMUM_LIQUIDITY, r);
}
function ammDeriveMinLiqNumsP2wpkh(poolIdBytes, maxCounter = 256) {
  for (let counter = 0; counter < maxCounter; counter++) {
    const ctrByte = new Uint8Array([counter & 0xff]);
    const xHash = sha256(concatBytes(_MIN_LIQ_DOMAIN_PUBKEY, poolIdBytes, ctrByte));
    const xBig = bytes32ToBigint(xHash);
    if (xBig >= SECP_P) continue;
    const candidate = concatBytes(new Uint8Array([0x02]), xHash);
    try {
      const pt = compressedPointFromHex(bytesToHex(candidate));
      if (pt.equals(PEDERSEN_ZERO)) continue;
      return hash160(candidate);
    } catch { /* try next */ }
  }
  throw new Error('MIN_LIQ NUMS recipient derivation exhausted');
}
// Verify an on-chain vout[k_min_liq] matches the deterministic MIN_LIQ
// construction. tx.vout[1] (vout[0] is the founder's lp-share output) MUST
// be a P2WPKH paying the NUMS-derived hash160. Returns true/false.
//
// Note on Pedersen commitment retrieval: the worker doesn't have direct
// access to the per-output Pedersen commit from chain inputs — it's
// committed in the envelope at the share commitment slot, but the locked
// MIN_LIQ output's Pedersen commitment lives in the asset-UTXO index that
// the worker maintains as it processes tx outputs. For POOL_INIT, we
// verify the structural P2WPKH match here; the commitment match is
// enforced by the asset-UTXO indexer's standard "vout commitment matches
// declared envelope amount" check when it processes vout[1].
function ammVerifyMinLiqVoutStructural(tx, poolIdBytes) {
  const vout1 = tx?.vout?.[1];
  if (!vout1 || typeof vout1.scriptpubkey !== 'string') return false;
  const spk = hexToBytes(vout1.scriptpubkey);
  // P2WPKH: 0x00 0x14 <20-byte program>
  if (spk.length !== 22 || spk[0] !== 0x00 || spk[1] !== 0x14) return false;
  let expectedP2wpkh;
  try { expectedP2wpkh = ammDeriveMinLiqNumsP2wpkh(poolIdBytes); }
  catch { return false; }
  if (expectedP2wpkh.length !== 20) return false;
  for (let i = 0; i < 20; i++) if (spk[2 + i] !== expectedP2wpkh[i]) return false;
  return true;
}
...
// computes the protocol's share of LP-fee growth since the last fee event
// and mints it into pool.protocol_fee_accrued (a virtual claim) while
// inflating lp_total_shares (diluting existing LPs). Swaps don't crystallize
// (fees accrue virtually via k = R_A·R_B growth).
//
// Pool fields touched: reserve_a, reserve_b, lp_total_shares,
//                      protocol_fee_accrued, k_last.
// Pool fields with no protocol fee enabled (zero-address) are no-op.
function _ammIsZeroAddressHex(addrHex) {
  if (typeof addrHex !== 'string' || addrHex.length !== 66) return false;
  for (let i = 0; i < 66; i++) if (addrHex.charCodeAt(i) !== 0x30) return false;
  return true;
}
function ammComputeProtocolShares({ S_pre, k_pre, k_now, protocol_fee_bps }) {
  const S = BigInt(S_pre), kPre = BigInt(k_pre), kNow = BigInt(k_now);
  const bps = BigInt(protocol_fee_bps);
  if (bps < 0n || bps > BigInt(AMM_PROTOCOL_FEE_BPS_MAX)) {
    throw new Error(`protocol_fee_bps out of range: ${bps}`);
  }
  if (bps === 0n) return 0n;
  if (kNow <= kPre) return 0n;
  if (S === 0n) return 0n;
  const rootPre = ammIsqrt(kPre);
  const rootNow = ammIsqrt(kNow);
...
// Accepts pool record as stored in KV (numbers are decimal strings) and
// returns a new pool record (also string-serialized) with the same shape.
// All BigInt arithmetic internally; only the public field types differ.
function ammCrystallizeProtocolFee(pool) {
  if (!pool) return pool;
  const addr = pool.protocol_fee_address;
  if (!addr || _ammIsZeroAddressHex(addr)) return pool;
  const bps = pool.protocol_fee_bps;
  if (!bps || bps === 0) return pool;
  const rA = BigInt(pool.reserve_a);
  const rB = BigInt(pool.reserve_b);
  const k_now = rA * rB;
  const k_pre = pool.k_last ? BigInt(pool.k_last) : 0n;
  if (k_now <= k_pre) {
    // No growth (or first call). Still set k_last for the baseline.
    return { ...pool, k_last: k_now.toString() };
  }
  const newShares = ammComputeProtocolShares({
  ...
  return {
    ...pool,
    protocol_fee_accrued: ((pool.protocol_fee_accrued ? BigInt(pool.protocol_fee_accrued) : 0n) + newShares).toString(),
    lp_total_shares: (BigInt(pool.lp_total_shares) + newShares).toString(),
    k_last: k_now.toString(),
  };
}
...
function decodeTProtocolFeeClaimPayload(payload) {
  ...
  p += 8;
  const claimCSecp = payload.slice(p, p + 33); p += 33;
  try { compressedPointFromHex(bytesToHex(claimCSecp)); } catch { return null; }
  const claimBlinding = payload.slice(p, p + 32); p += 32;
  const claimSig = payload.slice(p, p + 64); p += 64;
  return {
...
// T_CXFER_BPP structural decoder (SPEC §5.47 amendment). Byte-for-byte mirror
// of decodeCXferPayload except the opcode byte is 0x22 instead of 0x23. The
// rangeproof is opaque to the worker (Bulletproofs+ verification is client-
// side, same posture as standard BP).
...
function verifySchnorr(sig64, msgHash, pubXonly32) {
  ...
  // Defensive: noble's secp Point.multiply throws on scalar=0. BIP-340 doesn't
  // strictly forbid s=0, but in our envelope construction it can only arise
  // from adversarial / malformed sigs (a real signer's nonce + d combo would
  // be cryptographically negligible to hit). Treat s=0 and e=0 as the
  // identity-point contributions and let the final identity-check reject.
  const sG = sBig === 0n ? secp.ProjectivePoint.ZERO : secp.ProjectivePoint.BASE.multiply(sBig);
  const eP = e === 0n ? secp.ProjectivePoint.ZERO : P.multiply(e);
    ...
    for (const tx of txs) {
      ...
      if (decoded.opcode === T_CETCH) {
        ...
        if (lp.variant === 1) {
          ...
          // Launcher gate (AMM.md §"Optional launcher gate"). Each asset
          // MAY declare a `tacit_amm_launcher` pubkey in its IPFS-pinned
          // metadata blob (referenced by the CETCH's image_uri). If an
          ...
          if (gates.length > 0) {
            ...
            // Sigs ordered asset-A pubkey first, then asset-B (per spec).
            let allOk = true;
            for (let i = 0; i < gates.length; i++) {
              try {
                const sig = hexToBytes(lsigs[i]);
                if (sig.length !== 64) { allOk = false; break; }
                const pk = compressedPointFromHex(bytesToHex(gates[i].pubkey));
          ...
          // MINIMUM_LIQUIDITY locked-output check. Without this gate a
          // founder can send vout[1] to themselves and later T_LP_REMOVE
          // the entire pool — reopening the first-LP donation-inflation
          // attack. We verify vout[1] is the deterministic NUMS P2WPKH
          // derived from pool_id; the asset-UTXO indexer separately
          // enforces that vout[1]'s Pedersen commit equals the deterministic
          // C_min_liq when it indexes the output as an lp_asset_id UTXO.
          if (!ammVerifyMinLiqVoutStructural(tx, poolIdBytes)) continue;
          ...
          // Append to the canonical-pair reverse index so variant-0 LP_ADD
          // and T_LP_REMOVE can enumerate candidate pools at fee tiers /
          // capability_flags they don't carry in their wire format.
          await ammPairAppend(env, network, bytesToHex(aBytes), bytesToHex(bBytes), poolIdHex);
          found++;
        } else {
          ...
          // (AMM.md §"Envelope byte layouts" — variant-0 LP_ADD only carries
          ...
          let pool0 = null;
          let poolIdHex0 = null;
          let poolIdBytes0 = null;
          for (const candidate of v0Candidates) {
            const cPool = await ammPoolGet(env, network, candidate);
            if (!cPool) continue;
            ...
            // Provisional kernel-sig match (defer protocol-fee crystallization
            // + per-pool expectedShares to the matched pool below). We test
            // sig validity under THIS pool_id's kernel msg to disambiguate
            // candidates; the full validation runs against the matched pool.
            const trialOk = ammLpAddKernelVerify({
              variant: 0, poolId: cPoolIdBytes,
            ...
            if (!trialOk) continue;
            pool0 = cPool;
            poolIdHex0 = cPoolIdHex;
            poolIdBytes0 = cPoolIdBytes;
            break;
          }
          if (!pool0) continue;
          ...
          // (AMM_INITIAL_LP_LOCK_BLOCKS = 6 blocks per AMM.md §"Initial-LP
          // lock" — must match tests/amm-validator.mjs constant byte-for-byte.)
          if (typeof pool0.init_height === 'number') {
            if (h < pool0.init_height + AMM_INITIAL_LP_LOCK_BLOCKS) continue;
          }

          // Crystallize protocol fee BEFORE recomputing expectedShares. The
          // share-formula uses the post-crystallization lp_total_shares; LPs
          // joining a pool with non-zero protocol_fee_bps must query the
          // indexer's post-crystallization S for their envelope build. See
          // AMM.md §"Accrual model: Uniswap V2 lazy mintFee".
          const xPool0 = ammCrystallizeProtocolFee(pool0);

          // Expected shares: floor(min(ΔA·S/R_A, ΔB·S/R_B))
          let expectedShares;
          try {
            expectedShares = ammLpAddShares(
              dA0, dB0, xPool0.reserve_a, xPool0.reserve_b, xPool0.lp_total_shares,
            );
          } catch { continue; }
          ...
          // Kernel sig B-side (A-side already verified during candidate
          ...
          await ammPoolPut(env, network, poolIdHex0, {
            ...
            lp_total_shares: (BigInt(xPool0.lp_total_shares) + expectedShares).toString(),
        ...
        // Pool lookup: T_LP_REMOVE doesn't carry fee_bps / capability_flags
        // in its wire format. Enumerate candidates via the canonical-pair
        // reverse index and disambiguate via kernel-sig verification (which
        // binds the full pool_id including fee_bps + capability_flags).
        const rmCandidates = await ammPairGet(env, network, bytesToHex(aR), bytesToHex(bR));
        if (rmCandidates.length === 0) continue;
        ...
        let poolR = null;
        let poolIdHexR = null;
        let poolIdBytesR = null;
        let lpSide = null;
        let expected = null;
        for (const candidate of rmCandidates) {
          const cPool = await ammPoolGet(env, network, candidate);
          if (!cPool) continue;
          if (cPool.validation !== 'xcurve-verified'
              && cPool.validation !== 'verified') continue;
          ...
          // The LP-share asset_id is per-pool — distinguish by lp_asset_id.
          const cLpSide = rmInputsByAsset.get(cPool.lp_asset_id);
          if (!cLpSide || cLpSide.inputs.length === 0) continue;
          // Crystallize before computing expected (matches reference validator).
          const xCPool = ammCrystallizeProtocolFee(cPool);
          const totalShares = BigInt(xCPool.lp_total_shares);
          if (shareAmount === 0n || shareAmount > totalShares) continue;
          let cExpected;
          try { cExpected = ammLpRemoveOutputs(shareAmount, xCPool.reserve_a, xCPool.reserve_b, totalShares); }
          catch { continue; }
          if (cExpected.deltaA !== dARecv || cExpected.deltaB !== dBRecv) continue;
          if (cExpected.deltaA === 0n || cExpected.deltaB === 0n) continue;
          // Provisional kernel-sig check to disambiguate; full re-check below.
          const trialOk = ammLpRemoveKernelVerify({
            poolId: cPoolIdBytes, shareAmount,
            deltaA: cExpected.deltaA, deltaB: cExpected.deltaB,
            recvACSecpBytes, recvBCSecpBytes,
          ...
          if (!trialOk) continue;
          poolR = xCPool;
          poolIdHexR = cPoolIdHex;
          poolIdBytesR = cPoolIdBytes;
          lpSide = cLpSide;
          expected = cExpected;
          break;
        }
        ...
        await ammPoolPut(env, network, poolIdHexR, {
          ...
          reserve_a: newReserveA.toString(),
          reserve_b: newReserveB.toString(),
          lp_total_shares: (BigInt(poolR.lp_total_shares) - shareAmount).toString(),
          k_last: (newReserveA * newReserveB).toString(),
        ...
        const rClBig = bytes32ToBigint(cl.claim_blinding_bytes);
        if (rClBig >= SECP_N) continue;
        const cClExpected = pedersenCommit(cl.claim_amount_bigint, modN(rClBig));
        let cClActual;
        try { cClActual = compressedPointFromHex(cl.claim_c_secp); }
        catch { continue; }
        ...
        await ammPoolPut(env, network, cl.pool_id, {
          ...xPoolCl,
          protocol_fee_accrued: '0',
          last_protocol_fee_claim_txid: tx.txid,
          last_protocol_fee_claim_height: h,
        });
        found++;
        ...
        // Bulletproof m=2 range check on (C_change_or_sentinel, C_receipt_secp).
        // This closes the change-side inflation: without it a trader can
        // claim delta_in_total > a_in_real, mint C_change to commit to
        // (a_real - delta_in_total) mod n (wraps under BigInt), and inflate
        // asset A by Δ when spending the change UTXO. Mirrors the
        // reference validator's `bulletproofVerify` call in tests/swap-var.mjs.
        let cChangeForBP;
        ...
        const rangeProofBytes = hexToBytes(sv.range_proof);
        let bpOk = false;
        try {
          bpOk = bpRangeAggVerify([cChangeForBP, cReceiptPoint], rangeProofBytes);
        } catch { bpOk = false; }
        if (!bpOk) continue;
...
export {
  ...
  AMM_INITIAL_LP_LOCK_BLOCKS, AMM_MIN_BATCH_SIZE,
  ...
  // MINIMUM_LIQUIDITY locked-output verification.
  ammDeriveMinLiqBlinding, ammDeriveMinLiqCommitment,
  ammDeriveMinLiqNumsP2wpkh, ammVerifyMinLiqVoutStructural,
  // JCS canonicalization + launcher-pubkey extraction.
  ammJcsCanonicalize, ammExtractLauncherPubkey, ammFetchLauncherPubkeyForAsset,
  // Bulletproof verifier (range proofs for T_SWAP_VAR change-side).
  bpRangeAggVerify, bpRangeAggBatchVerify,
  ...
  async fetch(req, env, ctx) {
    ...
    if (url.pathname.startsWith('/amm/pool/') && req.method === 'GET') {
      ...
      try {
        ...
        // Surface BOTH the stored state and the post-crystallization view so
        // LPs can compute their envelope against the same S that the validator
        // will reach (AMM.md §"LP-side awareness"). The crystallized view is
        // identical to stored for pools without protocol fees.
        const xPool = ammCrystallizeProtocolFee(pool);
        const response = {
          ...pool,
          crystallized: {
