...
| Blinded-pubkey commits (canonical primitive) | 📝 Draft (round-1) | Promotes `SPEC-CBTC-TAC-AMENDMENT.md §5.36.7`'s blinded-pubkey commit construction (`commit = pubkey + blinding·G`) to a standalone normative primitive. Defines self-derived + ECDH-derived variants, an opcode anchor registry, soft-fork-additive rollout for shipped surfaces. Surfaces two privacy capabilities side-by-side: shielded-amount (current default) and shielded-amount-plus-shielded-address (opt-in via tacit address-format capability signal). Functionally equivalent to BIP-352 Silent Payments; mechanically identical to BIP-341 Taproot key tweaking. Zero new cryptography, zero on-chain cost, zero migration risk. No opcode reservation in itself — proposed opcode variants (`T_CXFER_STEALTH`, `T_AXFER_STEALTH`, etc.) follow in separate amendments per the registry. | Spec-only; first deployments already shipped under cBTC.tac §5.36.7 and farm amendment (launcher_commit + bonder_commit). Phased rollout for CXFER/AXFER recipients + mixer-withdraw + slot-burn recovery + AMM trader + orderbook maker tracked per the anchor registry in §C of the amendment. | [`spec/amendments/SPEC-BLINDED-PUBKEY-AMENDMENT.md`](./spec/amendments/SPEC-BLINDED-PUBKEY-AMENDMENT.md) |
