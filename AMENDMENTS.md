...
| Atomic multi-hop AMM routing (`T_SWAP_ROUTE`) | 📝 Draft (round-1) | Opcode `0x33`; settles a single trader's N-hop swap through up to N_HOPS_MAX distinct AMM pools atomically in one Bitcoin tx. Uniswap-V2-router parity for tacit. Pre-ceremony viable — reuses `T_SWAP_VAR`'s bulletproof rangeproof + sigma + kernel-sig stack; no Groth16, no ceremony coupling. Orthogonal to `T_TRADE_BATCH` (`0x39` — that opcode is cross-surface AMM↔orderbook; this one is multi-pool AMM↔AMM). | Reference impl deferred until routing UX demand justifies engineering work; ships as a follow-up amendment whenever a router lands. Wire format + validator algorithm sketched in the amendment file. | [`spec/amendments/SPEC-SWAP-ROUTE-AMENDMENT.md`](./spec/amendments/SPEC-SWAP-ROUTE-AMENDMENT.md) |
