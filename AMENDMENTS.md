...
| Worker bond + equivocation slash | 📝 Draft (round-1) | Three opcodes (`T_WORKER_BOND_OPEN` / `_CLOSE` / `T_WORKER_SLASH`; tentative `0x5B`/`0x5C`/`0x5D`). TAC bond locked in a no-spending-key Taproot escrow (cBTC.tac §5.35.2 pattern — no federation, no multisig). Permissionless `T_WORKER_SLASH` accepts two conflicting `T_INTENT_ATTEST` envelopes as evidence; the indexer zeros the bond's TAC value, attributes a configurable bounty (`SLASH_REPORTER_BOUNTY_BPS`, default 10%) to the reporter's declared vout, and burns the remainder (per-outpoint reattribution against the 21M fixed TAC cap). Closes the channel-layer accountability gap left explicitly out of scope by `SPEC-ORDERBOOK-CHANNEL-AMENDMENT` and `SPEC-TACIT-MESH-AMENDMENT`. Slash against a closed bond (worker walked with the TAC before evidence surfaced) sets the global `equivocator` flag but pays no bounty — public-good submission. No new cryptographic primitive, no new ceremony. Inherits cBTC.tac's conservation-exception pattern (§5.39.2 `SLASH_DETECTED`, §5.39.4 `T_SHARE_SLASH_CLAIM`). | Not started; depends on cBTC.tac §5.35.2 escrow construction being frozen. Ships as fast-follow after channel + mesh. | [`spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md`](./spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md) |
