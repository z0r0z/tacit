...
| Per-trade variable-amount AMM swap | 🔍 Round-1 review complete | `T_SWAP_VAR` (`0x32`); per-fill against-curve settlement reusing CXFER N=2 crypto from `T_AXFER_VAR`; tick-fan coordination layer for settler-chosen Δ; opt-in alongside `T_SWAP_BATCH` | Not started; depends on V1 AMM ceremony for pool init | [`SPEC-SWAP-VAR-AMENDMENT.md`](./SPEC-SWAP-VAR-AMENDMENT.md) |
