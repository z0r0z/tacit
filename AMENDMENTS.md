...
| Per-trade variable-amount AMM swap | ✅ Merged | `T_SWAP_VAR` (`0x32`); per-fill against-curve settlement reusing CXFER N=2 crypto from `T_AXFER_VAR`; tick-fan coordination layer for settler-chosen Δ; opt-in alongside `T_SWAP_BATCH`. No new Groth16 circuit, no new ceremony — uses existing AMM pool state pinned at POOL_INIT. **Revised 2026-06-05: outcome-taxonomy settlement** (market-order-within-floor + pass-through — curve evaluated at actual running reserves, `min_out` is the binding price consent, non-executable-but-authenticated envelopes refund the input at the receipt slot instead of consuming it without credit; declared `R_pre` / `delta_out` demote to advisory; credited receipt commitments are validator-derived from the published `r_receipt`; height-gated by `SWAP_VAR_OUTCOME_ACTIVATION`). Closes the concurrent-same-pool burn race structurally; `T_SWAP_ROUTE` (§5.22) inherits route-atomically. | Reference impl at `tests/swap-var.mjs` (44 tests); re-exported from `tests/amm-validator.mjs`; SPEC.md §5.20 ✅; spec-conformance pins 5 new domain tags + `OPCODE_T_SWAP_VAR == 0x32`. Extended-narrative draft preserved at the amendment file. 2026-06-05 revision: SPEC.md §5.20 + §5.5 dispatch + §5.22 rewritten ✅; reference validator + worker + dapp updates in progress. | [`spec/amendments/SPEC-SWAP-VAR-AMENDMENT.md`](./spec/amendments/SPEC-SWAP-VAR-AMENDMENT.md) |
...
| Cross-worker mesh attestation | 📝 Draft (round-1) | Passive browser-side cross-validation of `T_INTENT_ATTEST` from multiple workers. Catches selective worker censorship (one worker omits an intent from its published list while signing an attestation over the omitted set) by SHA-256 comparison of sorted intent-id lists across workers. Surfaces per-worker equivocation evidence directly to `T_WORKER_SLASH` per `SPEC-WORKER-BOND-AMENDMENT.md`. Phase-0-only; earlier WebRTC-relay and active-attester phases removed from ship path (see git history). No new opcode, no new domain tag, no new crypto. Pure dapp-side. | Not started; ships alongside orderbook channel + worker-bond | [`spec/amendments/SPEC-TACIT-MESH-AMENDMENT.md`](./spec/amendments/SPEC-TACIT-MESH-AMENDMENT.md) |
| Worker bond + equivocation slash | 📝 Draft (round-1) | Three opcodes (`T_WORKER_BOND_OPEN` / `_CLOSE` / `T_WORKER_SLASH`; tentative `0x5F`/`0x60`/`0x61` — shifted from prior `0x5B`/`0x5C`/`0x5D` once the preauth/offline-trading family block at `0x5B`–`0x5E` was reserved by `SPEC-PREAUTH-BID-AMENDMENT.md`). TAC bond locked via the **cBTC.tac §5.47 lien pattern**: standard worker-controlled P2TR carrying a TAC asset commit, with the lien recorded in a `bond_state` table that `commitmentForUtxo` refuses to honor unauthorized spends of (no covenant, no federation, no multisig — would reuse the cBTC.tac lien enforcement plumbing, which is **reserved/removed in V1**; see Implementation). Permissionless `T_WORKER_SLASH` accepts two conflicting `T_INTENT_ATTEST` envelopes as evidence; the indexer transfers the bond's lien-recorded TAC value to the reporter's declared vout (`SLASH_REPORTER_BOUNTY_BPS` default 10%) and burns the remainder (per-outpoint reattribution against the 21M fixed TAC cap). Cooperative close DOES spend `bond_outpoint` at the Bitcoin layer (worker single-sig key-path); slash does NOT (reporter doesn't have the worker's key). Closes the channel-layer accountability gap left explicitly out of scope by `SPEC-ORDERBOOK-CHANNEL-AMENDMENT` and `SPEC-TACIT-MESH-AMENDMENT`. Slash against a closed bond (worker withdrew before evidence surfaced) sets the global `equivocator` flag but pays no bounty — public-good submission. No new cryptographic primitive, no new ceremony. Inherits cBTC.tac's conservation-exception pattern from `T_SHARE_SLASH_CLAIM` (§5.39.4). | Not started. Its TAC-bond lien enforcement was to reuse the cBTC.tac §5.47 lien code, which is **removed from the indexer in V1** (the lien cBTC.tac model is reserved, not shipped) — so worker-bond needs its own lien-or-covenant enforcement before it ships. Fast-follow after channel + mesh. | [`spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md`](./spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md) |
...
| Self-custody slot wrapper (cBTC.zk) | ✅ Reference impl shipped + live signet validation; pending mainnet CETCH + POOL_INIT | Three opcodes: `T_SLOT_MINT` (`0x43`) + `T_SLOT_BURN` (`0x44`) + `T_SLOT_ROTATE` (`0x45`). Adds `tacit_wrapper.custody.kind = "self_custody_slot"` to §4.2. Per-note BTC UTXO at `K_btc = recipient_commit − denom·H`; the mixer note's `r_leaf` IS the BTC spending key, mathematically. No federation, no oracle, no co-signer, no escape — the structurally-most-trustless point on the wrapped-BTC curve. Lost notes lock backing sats permanently (same property native Bitcoin already has). (Legacy slot BURN/ROTATE used the mixer Groth16 ceremony; **V1: the cBTC.zk lock is verified by SP1 reflection** — Bitcoin tx parse + PoW + merkle inclusion — and backs the pool cBTC mint via `OP_CBTC_MINT`.) cBTC.zk is the entry-path lock for cBTC.tac (V1's real-BTC, oracle-free conservation peg) and the bootstrap target for AMM/orderbook BTC trading. | Worker validators + dispatch + log endpoints (`/slot-rotates`, `/slot-mints`, `/slot-splits`, `/slot-merges`) shipped in `worker/src/index.js`. Dapp encoders/decoders + builders + `verifySlotLeafOnChain` scanPools dispatch + `scanInboundSlotNotes` recipient-detection shipped in `dapp/tacit.js`. Slot-leaf depth-gate symmetric with mixer-leaf depth-gate (`_annotateNullifiers` + `status === 'included'` filter, SPEC §5.10 reorg-safety). Tests: `cbtc-zk-scanpools-leaf-verify.test.mjs` (22/22 — slot-leaf dispatch + 7 negative cases across all 4 slot kinds), `mixer-nullifier-depth-gate.test.mjs` (6/6 — pending/included/reorg-out transitions), `slot-wrapper.test.mjs` (107/107), `cbtc-zk-trading-flow.test.mjs` (44/44), `ceremony-vk-pin.test.mjs` (5/5 — pinned sha256s of vk/r1cs/ptau/zkey + TACIT_DEFAULT_CEREMONY_HASH). Live signet end-to-end: `tests/cbtc-zk-slot-lifecycle-signet.mjs` — all 6 phases pass against canonical-CID pool (POOL_INIT → MINT → scanPools → merkle proof → BURN with real Groth16 → double-spend pre-flight probe). Canonical wrapper metadata + first-minter procedure at `dapp/wrappers/cbtc-zk/README.md`. | [`spec/amendments/SPEC-CBTC-ZK-AMENDMENT.md`](./spec/amendments/SPEC-CBTC-ZK-AMENDMENT.md) |
| cBTC.zk fungibility (SPLIT / MERGE / NOTE) | ✅ Reference impl shipped; signet validation pending | Three opcodes: `T_SLOT_SPLIT` (`0x46`) + `T_SLOT_MERGE` (`0x47`) + `T_SLOT_NOTE` (`0x48`, embedded payload extension on slot-creating envelopes). Atomic 1→N denomination splits (`Σ denom_new ≤ denom_old`); atomic N→1 merges with multi-owner support; encrypted ECDH-AEAD recipient-detection memos (Sapling-style sender→viewing-key, AES-256-GCM with `tacit-slot-note-v1` domain tag, 122-byte ciphertext). Together with the base amendment these make cBTC.zk usable for paid distribution + recipient-detected sends without out-of-band coordination. | Worker validators for SPLIT/MERGE shipped (full nullifier-consume + leaf-append + per-output `encrypted_note` persistence on leaf records + chronological log entries via `/slot-splits` + `/slot-merges`). Dapp encoders/decoders + `buildSlotSplitEnvelope` + `buildSlotMergeEnvelope` shipped. `encryptSlotNote` / `decryptSlotNote` + viewing-key derivation shipped. Tests: `slot-split.test.mjs` (58/58), `slot-merge.test.mjs` (55/55), `slot-split-merge-builder.test.mjs` (72/72), `slot-note-encryption.test.mjs` (28/28). Signet-on-chain harness for SPLIT/MERGE deferred; unit + builder tests give end-to-end coverage of the envelope + crypto paths. | [`spec/amendments/SPEC-CBTC-ZK-FUNGIBILITY-AMENDMENT.md`](./spec/amendments/SPEC-CBTC-ZK-FUNGIBILITY-AMENDMENT.md) |
| cBTC.zk fractional (T_SLOT_FRACTIONALIZE / RECONSOLIDATE) | 📝 Draft — activates when Bitcoin gains covenants | Two opcode reservations: `0x4D` + `0x4E`. Two-key slot construction (`r_pedersen` + `r_btc` independent), Pedersen-sum identity for fractional shares, share-commit semantics, validator algorithms — cryptographic spec is complete. Only the Bitcoin-side enforcement (BIP-119 CTV / OP_VAULT / OP_CAT or similar covenant primitive) is missing, so the opcodes stay reserved and inactive until covenants land. In V1, cBTC.tac is a real-BTC, oracle-free **conservation peg** (`OP_CBTC_MINT`); the earlier (TAC, tETH)-bonded lien path (`SPEC-CBTC-TAC-AMENDMENT.md`) is **reserved, not shipped**. These fractional opcodes are the covenant-era Bitcoin-native successor. | No implementation — spec lives in the amendment file pending Bitcoin covenant activation. | [`spec/amendments/SPEC-CBTC-ZK-AMOUNT-AMENDMENT.md`](./spec/amendments/SPEC-CBTC-ZK-AMOUNT-AMENDMENT.md) |
| Buyer-offline preauth bid (`T_PREAUTH_BID`) | 📝 Draft (round-1) | Opcode `0x5B` (reassigned from initial `0x59` draft after spec-table reconciliation against shipped cBTC.tac extended block at `0x57`–`0x5A`); family head for the **preauth/offline-trading block** `0x5B`–`0x5E`. Round-2 update inlined `(bid_id, recipient_pubkey, amount, blinding, price_sats)` in the envelope so the validator + wallet recovery both work chain-only (+81 bytes per settlement, ~$0.10 mainnet, no bid-record dependency). Symmetric closure of the §5.7 trading matrix. §5.7.8 made the **ask** side offline-capable for the seller (seller pre-signs asset input with `SIGHASH_SINGLE_ACP`, buyer assembles); this amendment does the same for the **bid** side (buyer pre-signs sats input + canonical bid-context OP_RETURN with the same flag, seller assembles). Closes the "sells just work" UX gap — today every bid path (§5.7.7 bid intents, §5.7.7 variable-fill bids) requires the buyer to be online during the take window. New OP_RETURN binding rule (`OP_RETURN(32)` carrying full `bid_context_hash`, same shape as `T_SWAP_BATCH` / `T_WRAPPER_ATTEST` envelope commits; domain tag `tacit-preauth-bid-context-v1` lives inside the SHA256 input, not on chain — round-1 resolved against on-chain ASCII tag for consistency with the four existing tacit OP_RETURN forms) ties the buyer's pre-signed sats input to `(asset_id, recipient_pubkey, amount, blinding, price_sats)` — without it a vanilla `T_AXFER` would let a malicious seller take sats + redirect asset to themselves. Reuses `T_AXFER`'s kernel-sig + Pedersen + bulletproof stack verbatim; the only delta is the validator dispatch path enforcing the OP_RETURN binding. No new BIP-340 primitives, no new HMAC keystream domains, no ceremony coupling. Position-independence carries through from §5.7.8.1 (same `SIGHASH_SINGLE_ACP` BIP-143 invariant). Soft-fork-additive per §5.5 unknown-opcode rule. | Not started; ships independent of AMM ceremony. Reference impl track follows the §5.7.8 preauth-sale shape — wire-format encoder/decoder + validator dispatch + dapp publish/take builders + worker bid-book endpoints + signet e2e harness. | [`spec/amendments/SPEC-PREAUTH-BID-AMENDMENT.md`](./spec/amendments/SPEC-PREAUTH-BID-AMENDMENT.md) |
| Buyer-offline partial-fill preauth bid (`T_PREAUTH_BID_VAR`) | 📝 Draft (round-1) | Opcode `0x5C`; the "holy grail" set-and-forget partial-fill bid UX. Buyer publishes `(min_fill, max_fill, price_per_unit, fill_increment)` and K = (max-min)/inc + 1 `SIGHASH_SINGLE_ACP` pre-signatures, one per allowed fill ratio. Seller picks a fill chunk in `[min_fill, max_fill]`, attaches the matching pre-sig, settlement tx includes a PROTOCOL-ENFORCED refund vout for the unfilled portion. The K-pre-sig design avoids any new BIP-340 primitive, covenant requirement, or ceremony coupling — extends §5.7.11 with per-ratio inline section + K-sig bid record + two new validator rules (fill-ratio validity + refund-vout enforcement). On-chain cost per fill: +24 bytes vs §5.7.11; bid-record cost: +71 bytes × K (e.g., K=10 → 710 extra bytes off-chain). Mirrors §5.7.6.1 `T_AXFER_VAR` on the buyer-offline side. Single fill per bid in v1; repeated-fill same-bid is a follow-up. | Not started; ships after §5.7.11 baselines the wire format + validator dispatch + chain-only recovery. Reference impl track: encoder/decoder + K-sig batch verify + refund-vout validator + dapp publish/take builders + worker bid-book endpoints + signet e2e harness. | [`spec/amendments/SPEC-PREAUTH-BID-VAR-AMENDMENT.md`](./spec/amendments/SPEC-PREAUTH-BID-VAR-AMENDMENT.md) |
| Blinded-pubkey commits (canonical primitive) | ✅ Shipped for CXFER (class-2 stealth recipient) | Promotes `SPEC-CBTC-TAC-AMENDMENT.md §5.36.7`'s blinded-pubkey commit construction (`commit = pubkey + blinding·G`) to a standalone normative primitive. Defines self-derived + ECDH-derived variants, an opcode anchor registry, soft-fork-additive rollout for shipped surfaces. Round-2 review pass closed all paragraph-level items: §A.2.5 input-pubkey aggregation rule, §D.1 concrete bech32m encoding (HRPs `tcs`/`tcsts`/`tcsrt`), §D.2 explicit domain-tag dispatch, §D.3 light-client privacy trade-off load-bearing, §F.6 scan-flooding analysis, §F.7 fund-critical refusal test obligation, §G.5 BIP-352 byte-non-compatibility table, §H.5 historical-bootstrap. Class-1/class-2 split (round-1) means transfer-recipient stealth needs ZERO new opcodes — pure dapp+scanner change. Surfaces two privacy capabilities side-by-side: shielded-amount (current default) and shielded-amount-plus-shielded-address (opt-in via tacit address-format capability signal). Functionally equivalent to BIP-352 Silent Payments; mechanically identical to BIP-341 Taproot key tweaking. Zero new cryptography, zero on-chain cost, zero migration risk. | Reference primitives at `tests/stealth-primitives.mjs`; unit tests at `tests/stealth-math.test.mjs` (40/40), `tests/cxfer-stealth.test.mjs` (30/30), `tests/dapp-stealth-parity.test.mjs` (11/11), `tests/stealth-chain-fuzz.test.mjs` (11/11), `tests/stealth-credit-persistence.test.mjs` (15/15) — 107 stealth tests green. Signet round-trip e2e at `tests/stealth-signet-e2e.mjs` and `tests/stealth-cxfer-signet-e2e.mjs` (six phases including stealth→stealth chain; all green). Dapp integration shipped: `encodeStealthAddress`, `decodeStealthAddress`, `senderComputeStealthCommit`, `recipientScanTxForStealth`, `scanAssetForStealthReceipts`, persistent stealth-credit store, scanHoldings rehydration, Wallet card + per-asset Shielded address row, send-form HRP routing, Activity-feed SHIELDED badge, self-shielded broadcast auto-persist, once-per-session background auto-scan with negative-cache short-circuit, status pill. Worker endpoint `GET /assets/:asset_id/recent-xfer-txids` deployed. Backwards-compat audit at `spec/design/BLINDED-PUBKEY-COMPAT-AUDIT.md`. Operational roadmap at `spec/design/BLINDED-PUBKEY-ROADMAP.md`. First class-1 deployments (cBTC.tac §5.36.7, farm `launcher_commit`/`bonder_commit`) already shipped. Follow-ups: AXFER family stealth (T_AXFER / T_AXFER_VAR / T_SWAP_VAR / T_AXINTENT); mixer-withdraw / slot-burn `recovery_commit`; AMM `trader_commit`; orderbook `maker_commit`; QR code rendering for shielded address handoff. Each via own per-opcode amendment citing this primitive. | [`spec/amendments/SPEC-BLINDED-PUBKEY-AMENDMENT.md`](./spec/amendments/SPEC-BLINDED-PUBKEY-AMENDMENT.md) |
| Asset-settled liquidation bid (`T_PREAUTH_BID_ASSET`) | 📝 Draft (round-1) | Opcode `0x5E` (last slot of the preauth/offline-trading block `0x5B`–`0x5E`); asset-for-asset twin of `T_PREAUTH_BID_VAR` (`0x5C`) — K pre-generated Groth16 asset-spend proofs in place of K `SIGHASH_SINGLE_ACP` sigs, settling tacit-asset-for-tacit-asset (buyer pays cBTC.tac). Wires into `T_CBTC_TAC_FORCE_CLOSE` (`0x4B`) as a non-reflexive, covenant-free liquidation venue: seized bond converts to cBTC.tac via a competitive standing-bid auction (best-price-first, `LIQ_FLOOR_FRAC` reserve, walk-the-book partial fills), degrading to the §5.38 v1 early-slash floor when the book is thin. No new primitive / covenant / ceremony. | Not started; depends on §5.7.12 wire baseline + cBTC.tac §5.38/§5.39 insurance floor. | [`spec/amendments/SPEC-LIQ-BID-AMENDMENT.md`](./spec/amendments/SPEC-LIQ-BID-AMENDMENT.md) |
| cBTC.tac bond = (TAC, tETH) LP | 🪦 Superseded for V1 — reserved (Bitcoin-native lien bond NOT shipped; V1's cBTC.tac is the real-BTC conservation peg, `OP_CBTC_MINT`) | No new opcode — supersedes the §5.47 bond *composition*: the bond is an LP-share of the canonical **(TAC, tETH)** pool (neither leg is BTC), valued in BTC via **dual TWAP**, with all risk priced by **one governable, IL-aware-floored `BOND_RATIO`** (default 2.5×, band [2.0×, 5.0×]) + `MAX_POOL_FRAC` depth cap. Half-exogenous (tETH dilutes TAC reflexivity), **non-circular** (no cBTC.tac in the bond), fee-bearing, single homogeneous instrument → no weighting/segmentation/BTC-anchoring machinery. **Simpler genesis** (no cBTC.tac needed to mint the first cBTC.tac — just seed the (TAC, tETH) pool). cBTC.zk slot stays the real-BTC backing; **lien mechanism (§5.47) unchanged**. Governance gives TAC a value leg beyond bonding. Liquidation: force-close `T_LP_REMOVE`s → bids TAC + tETH legs for cBTC.tac via `0x5E`. Hard rules: IL-aware ratio floor; ratio keys on risk/depth, never price. | Not started; ships with tETH in V1 AMM. Worker dual-TWAP valuation + (TAC, tETH) pool acceptance is a tracked follow-up (§5.52.11; deposit flow foundation-only, not live). Depends on cBTC.tac §5.47 + tETH bridge. | [`spec/amendments/SPEC-CBTC-TAC-COLLATERAL-AMENDMENT.md`](./spec/amendments/SPEC-CBTC-TAC-COLLATERAL-AMENDMENT.md) |
| Bitcoin↔Ethereum reflection prover | ✅ LIVE (shipped) | Cross-lane SP1 reflection — proves a Bitcoin burn/lock and reflects it onto the Ethereum confidential pool; binds to the existing Bitcoin envelope (adds **no** Bitcoin opcode). Pins `BITCOIN_RELAY_VKEY`; gated on-chain by `attestBitcoinStateProven`. Backs the cross-chain ops (`OP_BRIDGE_MINT/BURN`, `OP_CBTC_MINT`, …). | Live in `ConfidentialPool.sol` + `BtcCallExecutor.sol` + the `cxfer-core` reflection guest. The 31 cross-chain ops it backs are tabulated in [`spec/SPEC-CONFIDENTIAL-OPS.md`](./spec/SPEC-CONFIDENTIAL-OPS.md). | [`spec/amendments/SPEC-BITCOIN-REFLECTION-AMENDMENT.md`](./spec/amendments/SPEC-BITCOIN-REFLECTION-AMENDMENT.md) |
| Ethereum-lane confidential pool + EVM confidential token | ✅ LIVE (as-built) | The cross-chain shielded pool — one secp256k1 Pedersen note across Bitcoin and Ethereum, Bulletproofs+ range proofs, **setup-free** (no ceremony) — plus the canonical ERC-20 hub the pool mints/burns against. The primary amount-flexible privacy + DeFi surface. | Live: `ConfidentialPool.sol`, `ConfidentialRouter.sol`, `CanonicalAssetFactory.sol`; op set in `spec/SPEC-CONFIDENTIAL-OPS.md`. | [`spec/amendments/SPEC-EVM-CONFIDENTIAL-TOKEN-AMENDMENT.md`](./spec/amendments/SPEC-EVM-CONFIDENTIAL-TOKEN-AMENDMENT.md) |
| Controller-vault: confidential CDP + fair farms | ✅ shipped | Generic confidential CDP (lock a collateral basket → mint a controller-derived debt note; **cUSD** is the first instance) + the fair-farm fold lifecycle, via guest ops `OP_CDP_MINT/CLOSE/LIQUIDATE/TOPUP` + `OP_FARM_BOND/HARVEST/UNBOND` + `OP_WRAP_CDP_MINT`. Pricing/ratio policy lives in the mutable `CollateralEngine`. | Live across the settle + reflection guests and `dapp/confidential-farm.js`/`confidential-pool.js`. | [`spec/amendments/SPEC-CONTROLLER-VAULT-AMENDMENT.md`](./spec/amendments/SPEC-CONTROLLER-VAULT-AMENDMENT.md) |
| Bitcoin-hook callback (Mode-B) | ✅ LIVE | Optional value-free contract callback fired off a reflected Bitcoin authorization (`T_BTC_CALL`); `BtcCallExecutor` records and one-shot-fires it. Adds no spend opcode — rides the reflection envelope. | Live: guest + `BtcCallExecutor.sol` + `dapp/confidential-btc-call.js`. | [`spec/amendments/SPEC-BITCOIN-HOOK-AMENDMENT.md`](./spec/amendments/SPEC-BITCOIN-HOOK-AMENDMENT.md) |
| tETH trustless ETH↔Bitcoin bridge | ✅ LIVE on mainnet — legacy | Standalone ETH→Bitcoin mixer-bridge (Groth16 deposit proof + SP1 burn proof). **Superseded for new flows** by the confidential-pool reflection bridge (tETH is now a pool asset); retained for note recovery/migration. | Live (`TacitBridgeMixer` + dapp/worker); status + addresses in [`BRIDGE.md`](./BRIDGE.md). | [`spec/amendments/SPEC-TETH-BRIDGE-AMENDMENT.md`](./spec/amendments/SPEC-TETH-BRIDGE-AMENDMENT.md) |
| TAC DAO governance | 📝 Drafted | `T_GOV_PROPOSAL` / `T_GOV_VOTE` / `T_GOV_VETO` / `T_GOV_EXECUTE` (`0x50`–`0x53`); TAC-weighted, safety-band-bounded parameter governance. | Drafted (SPEC.md §1.1 rows `0x50`–`0x53`). | [`spec/amendments/SPEC-GOVERNANCE-AMENDMENT.md`](./spec/amendments/SPEC-GOVERNANCE-AMENDMENT.md) |
...
- **2026-06-26** — **V1-lockdown index reconciliation.** Indexed the six
  shipped Ethereum-lane / cross-chain amendments that post-dated this file
  (Bitcoin↔Ethereum reflection prover, confidential pool + EVM confidential
  token, controller-vault CDP + fair farms, Bitcoin-hook callback, tETH
  bridge, TAC DAO governance), and marked the reflection + controller-vault
  amendments LIVE/shipped (were DRAFT). Marked the **cBTC.tac lien/CDP model
  superseded for V1 / reserved** — V1's fungible cBTC is the real-BTC,
  oracle-free conservation peg (`OP_CBTC_MINT`), so the lien opcodes
  (`SPEC.md` §1.1 `0x49`–`0x4F`/`0x57`–`0x5A`) are reserved (covenant-era) and
  the deprecated lien implementation was removed from the worker. Noted that
  V1's **cBTC.zk lock is SP1-reflection-verified** (not the mixer ceremony
  circuit). Added the canonical cross-chain opcode table
  ([`spec/SPEC-CONFIDENTIAL-OPS.md`](./spec/SPEC-CONFIDENTIAL-OPS.md)) and
  pruned stale/completed ops docs.
