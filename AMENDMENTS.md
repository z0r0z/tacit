...
| Range-attestation opcode | ✅ Reference impl shipped; SPEC.md §5.21 merge pending | `T_RANGE_ATTEST` (`0x44`); persistent on-chain range-attestation envelope binding a holder pubkey to a `commitment ≥ K` claim. Power-user feature (KYC tier proofs, reputation, governance weight). | Reference impl at `tests/range-attest.mjs` (23 tests); ships as V1.x amendment post-V1 ceremony. | [`spec/amendments/SPEC-RANGE-ATTEST-AMENDMENT.md`](./spec/amendments/SPEC-RANGE-ATTEST-AMENDMENT.md) |
