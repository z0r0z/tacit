...
| Worker bond + equivocation slash | 📝 Draft (round-1) | Three opcodes (`T_WORKER_BOND_OPEN` / `_CLOSE` / `T_WORKER_SLASH`; tentative `0x5F`/`0x60`/`0x61` — shifted from prior `0x5B`/`0x5C`/`0x5D` once the preauth/offline-trading family block at `0x5B`–`0x5E` was reserved by `SPEC-PREAUTH-BID-AMENDMENT.md`). TAC bond locked via the **cBTC.tac §5.47 lien pattern**: standard worker-controlled P2TR carrying a TAC asset commit, with the lien recorded in a `bond_state` table that `commitmentForUtxo` refuses to honor unauthorized spends of (no covenant, no federation, no multisig — reuses the same enforcement plumbing cBTC.tac already ships). Permissionless `T_WORKER_SLASH` accepts two conflicting `T_INTENT_ATTEST` envelopes as evidence; the indexer transfers the bond's lien-recorded TAC value to the reporter's declared vout (`SLASH_REPORTER_BOUNTY_BPS` default 10%) and burns the remainder (per-outpoint reattribution against the 21M fixed TAC cap). Cooperative close DOES spend `bond_outpoint` at the Bitcoin layer (worker single-sig key-path); slash does NOT (reporter doesn't have the worker's key). Closes the channel-layer accountability gap left explicitly out of scope by `SPEC-ORDERBOOK-CHANNEL-AMENDMENT` and `SPEC-TACIT-MESH-AMENDMENT`. Slash against a closed bond (worker withdrew before evidence surfaced) sets the global `equivocator` flag but pays no bounty — public-good submission. No new cryptographic primitive, no new ceremony. Inherits cBTC.tac's conservation-exception pattern from `T_SHARE_SLASH_CLAIM` (§5.39.4). | Not started; depends on cBTC.tac §5.47.3 `commitmentForUtxo` lien enforcement being live in the indexer (this is the same code path cBTC.tac already requires). Ships as fast-follow after channel + mesh. | [`spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md`](./spec/amendments/SPEC-WORKER-BOND-AMENDMENT.md) |
