...
fn main() {
    ...
    // ── OP_FARM_BOND: controller, owner, nonce, lp_asset, n_legs, [cx,cy,value,index,path,sigR,sigZ] ──
    {
    ...
    // ── OP_FARM_HARVEST: controller, owner, shares, nonce, harvest_nonce, reward, fee, lp_asset, old_index, old_path, reward(cx,cy,sigR,sigZ) ──
    {
        ...
        s.write(&hexv(f["lpAsset"].as_str().unwrap()));        // the STAKED asset the receipt commits (v3)
    ...
    assert_eq!(report.exit_code, 0, "guest REJECTED the witness (exit_code = {})", report.exit_code);
        println!("EXECUTE_OK farm_harvest cycles={} pv_bytes={} (receipt stays put + reward leaf + CdpMint debtValue==reward, receipt leaf in rateSnapshot)", report.total_instruction_count(), pv.as_slice().len());
    }

    // ── OP_FARM_UNBOND: controller, owner, shares, fee, nonce, lp_asset, old_index, old_path, release(cx,cy,sigR,sigZ) ──
    {
