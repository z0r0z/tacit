    ...
    (
        ...
        impl<'a, 'b> Mul<&'b $field> for &'a $field {
            ...
            fn mul(self, rhs: &'b $field) -> $field {
                #[cfg(all(target_os = "zkvm", feature = "zkvm-mulmod"))]
                {
                    return $field::zkvm_mulmod(self, rhs);
                }
        ...
        #[cfg(all(target_os = "zkvm", feature = "zkvm-mulmod"))]
        impl $field {
            /// Montgomery product via the uint256_mulmod precompile: (aR·bR mod p)·R^-1 mod p.
            /// R^-1 as an integer is the internal (Montgomery) form of R^-2 = (R3 as an element)^-1.
            #[inline(always)]
            pub fn zkvm_mulmod(&self, rhs: &$field) -> $field {
                static RINV: std::sync::OnceLock<[u64; 4]> = std::sync::OnceLock::new();
                let rinv = RINV.get_or_init(|| $r3.invert().unwrap().0);
                let mut x: [u64; 4] = self.0;
                let mut y = [0u64; 8];
                y[..4].copy_from_slice(&rhs.0);
                y[4..].copy_from_slice(&$modulus.0);
                unsafe { sp1_lib::syscall_uint256_mulmod(&mut x, y.as_ptr() as *const [u64; 4]) };
                y[..4].copy_from_slice(rinv);
                unsafe { sp1_lib::syscall_uint256_mulmod(&mut x, y.as_ptr() as *const [u64; 4]) };
                $field(x)
            }
        }
