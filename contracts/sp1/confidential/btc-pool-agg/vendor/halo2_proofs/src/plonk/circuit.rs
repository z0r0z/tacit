...
impl<F: Field> ConstraintSystem<F> {
    ...
    pub fn compress_selectors(self, selectors: Vec<Vec<bool>>) -> (Self, Vec<Vec<F>>) {
        self.compress_selectors_inner(selectors, true)
    }

    /// `compress_selectors` without materializing the combined selector polynomials.
    pub fn compress_selectors_verifier(self, selectors: Vec<Vec<bool>>) -> Self {
        self.compress_selectors_inner(selectors, false).0
    }

    fn compress_selectors_inner(mut self, selectors: Vec<Vec<bool>>, want_polys: bool) -> (Self, Vec<Vec<F>>) {
        ...
        let (polys, selector_assignment) = compress_selectors::process_inner(
            selectors
                .into_iter()
                .zip(degrees)
                .enumerate()
