...
pub fn process<F: Field, E>(
    ...
    max_degree: usize,
    allocate_fixed_column: E,
) -> (Vec<Vec<F>>, Vec<SelectorAssignment<F>>)
where
    E: FnMut() -> Expression<F>,
{
    process_inner(selectors, max_degree, allocate_fixed_column, true)
}

/// As `process`; with `want_polys = false` the combination polynomials are left empty (a
/// verifier needs only the selector assignments). The exclusion test runs on packed words.
pub fn process_inner<F: Field, E>(
    ...
    E: FnMut() -> Expression<F>,
{
    ...
    selectors.retain(|selector| {
        if selector.max_degree == 0 {
            ...
            let combination_assignment = if want_polys {
                selector
                    .activations
                    .iter()
    ...
    let packed: Vec<Vec<u64>> = selectors
        .iter()
        .map(|selector| {
            let mut w = vec![0u64; (selector.activations.len() + 63) / 64];
            for (k, b) in selector.activations.iter().enumerate() {
                if *b {
                    w[k / 64] |= 1 << (k % 64);
                }
            }
        ...
        .collect();
    for (i, rows) in packed.iter().enumerate() {
        // Loop over the selectors previous to this one
        for (j, other) in packed.iter().enumerate().take(i) {
            // Look at what selectors are active at the same row
            if rows.iter().zip(other.iter()).any(|(l, r)| l & r != 0) {
    ...
    for (i, selector) in selectors.iter().enumerate() {
        ...
        let mut combination_assignment = if want_polys { vec![F::ZERO; n] } else { vec![] };
