...
mod relay_fee_kernel_tests {
    ...
    fn refund_fee_must_be_bounded_by_the_locked_value() {
        ...
        // locker can open. The fee-bearing kernel alone ACCEPTS it — the gap the guard below exists to close.
...
pub fn lp_add_kernel_verify(
    ...
    input_commitments: &[Point],
    sig: &[u8; 64],
    // Refund binding signed by BOTH variants. `share_amount` doubles as the LP's minimum acceptable
    // shares: a variant-0 add that would mint fewer (a sandwich) returns the contributed `delta_x` to a fresh
    // note at `refund_dest_xonly` instead of absorbing the deposit, and `expiry_height` refunds a stale add the
    // same way; a POOL_INIT (variant 1) that loses the deterministic pool_id to a front-run (or is otherwise
    // stale/malformed post-kernel) returns its seeded `delta_x` the same way. Both refund terms + the deadline
    // are signed here (per side) so a relay can neither redirect the refund nor replay a stale op.
    expiry_height: u32,
    ...
    // (the refund tail is bound by both variants — variant 0 refunds a sandwiched/stale add, variant 1 a
    // front-run/stale POOL_INIT seed).
    let mut h = Sha256::new();
...
/// Memo binding: the running keccak over the per-leaf memo hashes, mirroring ConfidentialPool's
...
/// `getParentEnvelopeData` bid arms (tacit.js): the OP_RETURN-at-vout-0 layout of other reveals is NOT
/// used here — for bids the buyer FILLED note is at vout 0 and the seller CHANGE at a fixed later
...
pub fn btc_note_spend_msg(
    ...
    // out_leaves is the one variable-length field in this message, so its count is length-prefixed (u64 BE)
    // ahead of the leaves themselves. This makes the encoding injective by construction — no byte sequence
    // maps to two different out-counts — independent of any fixed field's width. Any further variable-length
    // field added here must be length-prefixed the same way. Mirror any change here in
    // dapp/confidential-pool.js btcNoteSpendMsg IN LOCKSTEP.
    k.update(&(out_leaves.len() as u64).to_be_bytes());
    for l in out_leaves {
        k.update(l);
    }
...
// instance; a single-leg basket (n=1) is the simple case, n>1 the multi-collateral one.
...
/// debt, and seize the owner's collateral as bearer notes they chose the blinding for.
...
/// EVM AMM pool_id WITH an optional protocol-fee skim. `protocol_fee_bps == 0` returns
...
/// Per-swap protocol (creator) fee cut — the protocol fee-switch realized per swap (NOT the lazy √k mintFee):
...
mod bridge_burn_id_tests {
    ...
    fn bridge_mint_reconstructs_every_burned_source_class() {
        ...
        // Class 2 — generation-bound reflected note (fold_burn records it under btc_note_leaf_bound; only
        // source_class 2 reconstructs it). Without a class-2 branch the mint could never reproduce this
        // leaf+burn_id, and the burn would strand the note.
...
/// bridge-mint round-trip above, applied to the bound-leaf seam.
#[cfg(test)]
mod cross_guest_roundtrip_tests {
    ...
    // BOUND note: same round-trip through the _bound producers/consumers, plus the
    ...
    // Pins the exact wire layout — domain ‖ chain_binding ‖ op_id ‖ in_leaf ‖ in_nullifier ‖
    // count(u64 BE) ‖ leaves ‖ fee(u64 BE) ‖ deadline(u64 BE) — against a from-scratch encoding, so the
    // out-count prefix can never silently drop out from under dapp/confidential-pool.js btcNoteSpendMsg,
    // which mirrors this same layout byte for byte.
    #[test]
    fn spend_msg_matches_length_prefixed_wire_layout() {
        let cb = [0x11u8; 32];
        let op = [0x22u8; 32];
        let in_leaf = [0x33u8; 32];
        let in_nu = [0x44u8; 32];
        let l0 = [0xB0u8; 32];
        let l1 = [0xB1u8; 32];
        let count = (2u64).to_be_bytes();
        let fee = (7u64).to_be_bytes();
        let deadline = (12345u64).to_be_bytes();
        let expected = kn(&[
            BTC_NOTE_SPEND_DOMAIN, &cb, &op, &in_leaf, &in_nu, &count, &l0, &l1, &fee, &deadline,
        ]);
        assert_eq!(btc_note_spend_msg(&cb, &op, &in_leaf, &in_nu, &[l0, l1], 7, 12345), expected);
    }

    // The one collision an unprefixed encoding could ever admit: a 2-leaf message whose second leaf equals
    // the (fee‖deadline) tail bytes of a 1-leaf message. The count prefix rules it out structurally — the
    // two constructions below never share a byte-length, let alone a byte string, at any (fee, deadline)
    // choice — rather than by any property of the fixed fields' widths.
    #[test]
    fn spend_msg_out_count_is_unambiguous_at_the_tail_boundary() {
        let cb = [0x55u8; 32];
        let op = [0x66u8; 32];
        let in_leaf = [0x77u8; 32];
        let in_nu = [0x88u8; 32];
        let l0 = [0xC0u8; 32];
        // Craft a "leaf" whose bytes are exactly a (fee, deadline) pair, and check it can never be
        // mistaken for a shorter output list plus that fee/deadline.
        let fee: u64 = 0x0102030405060708;
        let deadline: u64 = 0x0910111213141516;
        let mut l1 = [0u8; 32];
        l1[..8].copy_from_slice(&fee.to_be_bytes());
        l1[8..16].copy_from_slice(&deadline.to_be_bytes());
        let two_leaves = btc_note_spend_msg(&cb, &op, &in_leaf, &in_nu, &[l0, l1], 999, 999);
        let one_leaf_same_tail = btc_note_spend_msg(&cb, &op, &in_leaf, &in_nu, &[l0], fee, deadline);
        assert_ne!(two_leaves, one_leaf_same_tail, "out-count prefix makes the two encodings unequal length");
    }
}
...
/// Lazy-`mintFee` protocol-share crystallization (mirrors worker `ammComputeProtocolShares`):
...
/// Derive the full AMM pool_id, mirroring the worker's `ammDerivePoolId`:
/// `sha256(domain ‖ low ‖ high ‖ fee_bps_LE(2) ‖ capability_flags [‖ protocol_fee_address ‖
/// protocol_fee_bps_LE(2)])`, the fee suffix present iff `protocol_fee_bps != 0`. POOL_INIT carries `fee_bps`,
...
pub fn amm_derive_pool_id_full(
    ...
    // Mirrors the worker `ammDerivePoolId`. (capability_flags is a Bitcoin-side concept; the EVM `pool_id`
    // carries no flags byte.)
    if protocol_fee_bps != 0 {
...
pub struct PoolReserveState {
    ...
    // Protocol-fee (lazy √k `mintFee`) state, set at POOL_INIT from the 6-arg pool_id config. This is
...
impl PoolReserveState {
    /// Crystallize the lazy protocol fee from the SWAP-driven k-growth since `k_last`: mint the skim
    ...
    /// taxed as a fee (worker `ammCrystallizeProtocolFee`). No-op for a no-skim pool.
...
pub struct ScanReflection {
    ...
    // Per-position reward-per-share checkpoints (receipt leaf → entry rps), STAMPED at execution because the live
...
impl ScanReflection {
    ...
    pub fn fold_consumed(
        &mut self,
        nu: &[u8; 32],
        // The Ethereum-recorded consumed value = keccak(btc_spend_root ‖ source leaf), where the source leaf is
        // btc_note_leaf_bound(asset,Cx,Cy,auth_key,chain_binding) for a bound note or btc_note_leaf(asset,Cx,Cy,
        // auth_key) for a legacy one — the FULL authenticated source leaf, matching the contract's keccak256(spendRoot ‖
    ...
    ///
    /// Without the conservation gate the per-output `fold_output` loop would append an
    ...
    pub fn fold_swap_var(
        ...
        // The CHANGE output's REAL scriptPubKey (confirmed tx, vout 2) — bound by the intent exactly like the
        ...
        //     witness), so rejecting an unauthorized swap cannot censor an honest one. Both scripts are taken
        //     verbatim, never reconstructed from an assumed output type: synthesizing a program here would
        //     reject every honest swap whose emitter pays a different shape, after its input was already
        //     nullified by the vin scan — a total loss of the taker's principal.
        //     The CHANGE destination is bound the same way, because the change is onboarded as a real note too
        //     (below): binding only its commitment would let a settler pay the taker's leftover to its own
        //     script and still reproduce the signed message.
        let receive_spk = receive_spk.ok_or("swap_var fold: no receipt output at vout 1")?;
        // Sentinel change ⇒ no change note and no change output; the bound script is empty. Derived from the
        // envelope, so the message shape is never the settler's choice.
        let change_is_sentinel = env.c_change_or_sentinel.iter().all(|&b| b == 0);
        let change_spk_bound: &[u8] = if change_is_sentinel {
            &[]
        } else {
        ...
        // Same for the taker's change note, when there is one (non-sentinel): a non-P2TR change output would
        // onboard an unspendable change note.
        ...
        //     placeholder. The RECEIPT does not ride this aggregate: the guest forms it from a
    ...
    /// VALUE CHAIN, computed end-to-end and staged before any mutation (all-or-nothing):
    ...
    ///   - each later hop's input is the amount the PREVIOUS hop actually cleared, of that hop's output asset
    ///     — declared magnitudes are never read, so no value is conjured between pools (the intermediate
    ///     flows pool-to-pool, never a note);
    ///   - every pool is `c0_backed` and each hop is re-cleared at its CURRENT reserves with its registry fee
    ///     tier (`get_amount_out`, strictly below the out-side reserve);
    ///   - the receipt is FORMED in-guest from the final cleared amount under the PUBLIC `r_receipt`.
    /// A route that clears to nothing, misses the trader's signed `min_out`, or has expired is REFUNDED
    /// (`onboard_btc_refund`) rather than skipped, since the vin scan has already nullified its input. A pool
    /// repeated within one route is rejected (its staged reserves would be stale); such an exotic route just
    /// doesn't bridge directly (heal via a CXFER hop) — fail-closed, never an over-mint. See
    /// ops/DESIGN-bridge-multiasset-provenance.md (Track B).
    ...
    /// ownership, not a `C_0`-rooted asset note — the caller onboards it via `fold_lp_share_mint`); this fold
    /// only advances the reserve provenance that `fold_swap_var` consumes. Fails closed (changes nothing) on
    /// a bad kernel, an asset mismatch, or an unknown LP-add pool; a duplicate/expired/malformed POOL_INIT
    /// and a sandwiched/expired/zero-mint LP-add take the REFUND path instead (see below).
    #[allow(clippy::too_many_arguments)]
    pub fn fold_lp_add(
        &mut self,
        variant: u8,
        ...
        match variant {
            1 => {
                ...
                // A post-kernel failure — a front-run that already registered this deterministic pool_id, a
                // stale (expired) init, or a malformed seed — returns the founder's seeded delta_a / delta_b to
                // the bound refund dests instead of self-burning them (the vin scan already nullified the funding
                // notes, so a plain reject would strand the seed). Both refund outputs must be P2TR so their
                // notes carry a real spend authority; checked UNCONDITIONALLY, since a refund to an unspendable
                // dest strands exactly the principal this exists to protect.
                if refund_a_xonly == &[0u8; 32] || refund_b_xonly == &[0u8; 32] {
                    return Err("lp_add fold: POOL_INIT refund output is not P2TR (refund note would have no spend authority)");
                }
                // Initial total shares = isqrt(Δa·Δb), the geometric-mean first mint (the founder gets that minus
                // MINIMUM_LIQUIDITY, which stays locked — so TOTAL outstanding is isqrt). Tracks lp_total_shares.
                // First-mint floor (mirror EVM OP_LP_ADD): the founder's onboardable share is
                // `total_shares − MINIMUM_LIQUIDITY`; a seed whose geometric mean isn't strictly above the locked
                // floor mints zero/underflowing founder shares. The protocol skim (`10000 − protocol_fee_bps`)
                // and the swap-fee tier (`10000 − fee_bps` in get_amount_out) each underflow above their bound.
                // ALL of these are malformed-seed conditions — conserved via refund, not burned.
                let total_shares = isqrt(delta_a as u128 * delta_b as u128);
                let malformed = delta_a == 0
                    || delta_b == 0
                    || total_shares > u64::MAX as u128
                    || total_shares <= AMM_MINIMUM_LIQUIDITY as u128
                    || share_amount != (total_shares as u64).saturating_sub(AMM_MINIMUM_LIQUIDITY)
                    || protocol_fee_bps >= 10000
                    || fee_bps > AMM_MAX_POOL_FEE_BPS;
                let expired = expiry_height == 0 || (expiry_height as u64) < current_height;
                if self.pools.get(pool_id).is_some() || expired || malformed {
                    // Atomic + proof-fatal (see onboard_lp_refund_pair): both refund notes land together or the
                    // bad-witness-only proof is rejected — never a partial refund with the seed already retired.
            ...
            0 => {
                ...
                // Crystallize the protocol fee from swap-driven k-growth BEFORE the deposit (lazy
                // `mintFee`): `total_shares` may grow, so the proportional mint is over the POST-crystallization
    ...
    /// (2) the payout `delta_X = floor(R_X·share/S)` is RECOMPUTED here from the reserves and share supply as
    /// they stand (the envelope's declared pair is what the kernel signs, not what gets paid), so the
    /// reflection's reserves stay in lockstep with the worker's accounting and a later swap never desyncs;
    /// (3) each `recv_X` is FORMED in-guest as `delta_X·H + r_recv_X·G` from the recomputed payout under the
    /// PUBLIC on-wire blinding `r_recv_X`, so its value is EXACTLY the reserve decrease by construction (no
    /// over-withdrawal mints unbacked value) and the note tree stays prover-independent. Then reserves +
    /// shares are drawn down. A leg that rounds to zero re-mints the burned shares to the vout-2 refund note
    /// instead; fails closed on any other miss. See ops/DESIGN-bridge-multiasset-provenance.md (B).
    #[allow(clippy::too_many_arguments)]
    pub fn fold_lp_remove(
        ...
        // Crystallize the protocol fee from swap-driven k-growth BEFORE the withdrawal (lazy `mintFee`),
    ...
    /// with pool_ids — different derivations). A later `T_LP_HARVEST` draws reward notes from it.
    ///
    ...
    pub fn fold_farm_init_rewards(&mut self, farm_id: &[u8; 32], rate: u64, launcher_pubkey: &[u8; 33], pool_id: &[u8; 32], start_height: u64, end_height: u64, reward_total: u64) -> Result<(), &'static str> {
        ...
        st.start_height = start_height; // accrual clamps to [start, end] (end == 0 was rejected above)
    ...
    pub fn fold_lp_unbond(
        ...
        // The receipt commits the farm's pinned stake asset, so an unbond cannot re-label the
    ...
    /// LP-share asset). See ops/DESIGN-bridge-multiasset-provenance.md.
    ...
    pub fn fold_crossout(
        ...
        if c_low_value == claim_id {
            ...
            // ABORT rather than skip — a skippable Err here would let the dispatcher drop the mint.
            assert!(
...
pub struct FarmRewardState {
    ...
    // Σ over live positions of `shares_i · entry_rps_i` — the reward debt, in rps units. Bumped by
...
/// Witnessed map `receipt leaf → entry rps` — the per-position reward-per-share checkpoint, STAMPED at execution
...
mod tests {
    ...
    // fold_lp_bond / fold_lp_harvest / fold_lp_unbond run — proven end-to-end against an in-test
    ...
    // fold_lp_harvest (bounds reward vs the live rps and re-stamps the receipt's entry in place)
    ...
    /// StaleReflectionDigest). The contract constant must equal the ScanReflection genesis below — not
    /// any other reflection model's genesis.
    #[test]
    ...
    /// so a relayer cannot append an arbitrary spendable leaf into bitcoinPoolRoot. It must
    ...
    fn range_adversarial_edge_cases() {
        ...
        // before verify_range is ever called). Fed directly, `compress(identity)` yields the all-zero
        // 33-byte sentinel (the identity has no SEC1 33-byte form), so the transcript absorbs it and the
        // MSM cannot close — a reject, never a spoofed `true`. Assert the call cannot return true (a
        // fail-closed panic is tolerated too).
    ...
    fn fold_swap_var_gates_reject_inflation() {
        ...
        // The CHANGE output's scriptPubKey, likewise bound verbatim. `base` carries a non-sentinel change, so
        // every case below has a change note to onboard and a change destination to bind.
        const CHANGE_SPK: [u8; 22] = {
            let mut s = [0x44u8; 22];
            s[0] = 0x00;
            s[1] = 0x14;
            s
        ...
        // A REAL genesis append path: the expiry cases below REFUND rather than reject, so they reach an
        // append.
        ...
        // Deliberately NOT a gate: the swap's DECLARED r_a_pre/r_b_pre need not equal the tracked reserves.
        // Requiring that would strand principal — the loser of any two concurrent swaps on one pool would have
        // its input nullified by the vin scan and then its fold skipped, destroying the principal. A moved pool
        // is re-cleared at the current price, so the only thing bounding the trader is their signed min_out.
    ...
    fn lp_add_kernel_fixture(
        ...
        expiry: u32, refund_xonly: &[u8; 32], refund_blinding: &[u8; 32],
    ) -> (Point, [u8; 64]) {
        let r = scalar_reduce_be(r_seed);
        let c_in = gen_h() * Scalar::from(delta) + ProjectivePoint::generator() * r;
        let msg = lp_add_msg_v0(variant, pool_id, asset, delta, share_amount, share_csecp, &[outpoint], expiry, refund_xonly, refund_blinding);
    ...
    fn fold_protocol_fee_claim_crystallizes_and_onboards() {
        ...
        // POOL_INIT founder-refund binding (accept path leaves it unused, but the kernel binds it + the P2TR
        // gate requires non-zero dests + a live deadline).
        let (rax, rbx) = ([0x33u8; 32], [0x34u8; 32]);
        let (rab, rbb) = ([0x35u8; 32], [0x36u8; 32]);
        let exp = 100u32;
        let seed_pool = || {
            let (c_in_a, sig_a) = lp_add_kernel_fixture(1, &pool_id, &a, delta_a, founder, &share_csecp, op_a, &[0x71u8; 32], exp, &rax, &rab);
            let (c_in_b, sig_b) = lp_add_kernel_fixture(1, &pool_id, &b, delta_b, founder, &share_csecp, op_b, &[0x72u8; 32], exp, &rbx, &rbb);
            let mut sc = ScanReflection::genesis();
            sc.fold_lp_add(
                ...
                0, exp, &rax, &rbx, &rab, &rbb, &[0u8; 32], &[0u8; 32], &[], &[],
        ...
        // valid self-sig is rejected (a matching id is what proves recipiency).
    ...
    fn lp_add_msg_v0(variant: u8, pool_id: &[u8; 32], asset_x: &[u8; 32], delta_x: u64, share_amount: u64, share_csecp: &[u8; 33], inputs: &[([u8; 32], u32)], expiry: u32, refund_xonly: &[u8; 32], refund_blinding: &[u8; 32]) -> [u8; 32] {
        ...
        // The refund tail is bound by BOTH variants (variant 0 refunds a sandwiched/stale add, variant 1 a
        // front-run/stale POOL_INIT seed) — mirrors lp_add_kernel_verify.
    ...
    fn lp_add_kernel_verify_and_fold_pool_init() {
        ...
        let z = [0u8; 32];
        let ep: &[[u8; 32]] = &[];
        // POOL_INIT founder-refund binding (now signed by variant 1 too): non-zero P2TR dests + a live deadline
        // keep a well-formed init on the ACCEPT path; a malformed/duplicate init routes the seed to a refund.
        let (rax, rbx) = ([0x33u8; 32], [0x34u8; 32]); // refund destination x-only keys
        let (rab, rbb) = ([0x35u8; 32], [0x36u8; 32]); // refund note blindings
        let (rout_a, rout_b) = ([0x01u8; 32], [0x02u8; 32]); // refund note outpoints
        let exp = 100u32;
        // Valid append paths for the two founder-refund notes onboarded from a fresh (note_count 0) reflection.
        let refund_paths = |da: u64, db: u64| -> (Vec<[u8; 32]>, Vec<[u8; 32]>) {
            let leaf_a = reflected_note_leaf(&asset_a, &pedersen_commit_compressed(da, &rab), &rax).unwrap();
            let mut acc = KeccakTreeAccumulator::new();
        ...
        let (_pa, sa) = bip340_sign(&[0x41u8; 32], &[0x51u8; 32], &lp_add_msg_v0(1, &pid, &asset_a, da, share, &csc, &op_a, exp, &rax, &rab));
        let op_b = [([0x72u8; 32], 1u32)];
        let xb = scalar_reduce_be(&[0x42u8; 32]);
        let cib = gen_h() * Scalar::from(db) + ProjectivePoint::generator() * xb;
        let (_pb, sb) = bip340_sign(&[0x42u8; 32], &[0x52u8; 32], &lp_add_msg_v0(1, &pid, &asset_b, db, share, &csc, &op_b, exp, &rbx, &rbb));

        // kernel primitive: accept + reject tamper / wrong delta.
        assert!(lp_add_kernel_verify(1, &pid, &asset_a, da, share, &csc, &op_a, &[cia], &sa, exp, &rax, &rab), "valid A kernel");
        let mut bad = sa; bad[63] ^= 1;
        assert!(!lp_add_kernel_verify(1, &pid, &asset_a, da, share, &csc, &op_a, &[cia], &bad, exp, &rax, &rab), "tampered A sig rejected");
        assert!(!lp_add_kernel_verify(1, &pid, &asset_a, da + 1, share, &csc, &op_a, &[cia], &sa, exp, &rax, &rab), "wrong delta_a rejected");

        // POOL_INIT (backed): pool registered with the seeded reserves + c0_backed.
        let mut sc = ScanReflection::genesis();
        assert!(sc.fold_lp_add(1, &pid, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa, &op_b, &[cib], &sb, true, 0, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, ep, ep).is_ok(), "POOL_INIT folds");
        ...
        assert_eq!(sc.note_count, 0, "a well-formed POOL_INIT onboards no refund note");
        ...
        assert!(
            sc_fee.fold_lp_add(1, &pid, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa, &op_b, &[cib], &sb, true, 30, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, ep, ep).is_ok(),
        ...
        // A tier above the pool maximum is a malformed seed: instead of self-burning it, the founder's delta_a /
        // delta_b are returned as two owner-bound refund notes and NO pool is registered (conservation).
        let mut sc_cap = ScanReflection::genesis();
        let (pa, pb) = refund_paths(da, db);
        assert!(
            sc_cap.fold_lp_add(1, &pid, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa, &op_b, &[cib], &sb, true, AMM_MAX_POOL_FEE_BPS + 1, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, &pa, &pb).is_ok(),
            "POOL_INIT above the pool fee maximum takes the refund path",
        );
        assert!(sc_cap.pools.get(&pid).is_none(), "malformed POOL_INIT registers no pool");
        assert_eq!(sc_cap.note_count, 2, "malformed POOL_INIT refunds the seed as two notes");
        assert_eq!(sc_cap.live.get(&rout_a).unwrap().1, asset_a, "refund A rides asset A");
        assert_eq!(sc_cap.live.get(&rout_b).unwrap().1, asset_b, "refund B rides asset B");
        // duplicate POOL_INIT (a front-run already holds this deterministic pool_id) → refund, pool unchanged.
        let (pa, pb) = refund_paths(da, db);
        assert!(sc.fold_lp_add(1, &pid, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa, &op_b, &[cib], &sb, true, 0, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, &pa, &pb).is_ok(), "duplicate POOL_INIT refunds the seed");
        assert_eq!(sc.note_count, 2, "the front-run seed is refunded as two notes");
        assert_eq!(sc.pools.get(&pid).unwrap().reserve_a, da, "the winning pool's reserves are untouched");

        // bad B kernel → fold nothing (fresh pool id).
        let pid2 = [0x11u8; 32];
        let (_x, sa2) = bip340_sign(&[0x41u8; 32], &[0x51u8; 32], &lp_add_msg_v0(1, &pid2, &asset_a, da, share, &csc, &op_a, exp, &rax, &rab));
        let mut sc2 = ScanReflection::genesis();
        assert!(sc2.fold_lp_add(1, &pid2, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa2, &op_b, &[cib], &[0u8; 64], true, 0, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, ep, ep).is_err(), "bad B kernel rejected");
        ...
        let pid3 = [0x12u8; 32];
        let (_y, sa3) = bip340_sign(&[0x41u8; 32], &[0x51u8; 32], &lp_add_msg_v0(1, &pid3, &asset_a, da, share, &csc, &op_a, exp, &rax, &rab));
        let (_z, sb3) = bip340_sign(&[0x42u8; 32], &[0x52u8; 32], &lp_add_msg_v0(1, &pid3, &asset_b, db, share, &csc, &op_b, exp, &rbx, &rbb));
        let mut sc3 = ScanReflection::genesis();
        assert!(sc3.fold_lp_add(1, &pid3, &asset_a, &asset_b, da, db, share, &csc, &op_a, &[cia], &sa3, &op_b, &[cib], &sb3, false, 0, 0, 0, 0, exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, ep, ep).is_ok());
    ...
    fn lp_add_variant0_min_shares_expiry_and_refund() {
        ...
        let seed_exp = 100u32; // POOL_INIT deadline (variant 1 now binds + enforces expiry + refund dests)
        ...
        let seed = |ra: u64, rb: u64| -> ScanReflection {
            ...
            let op_a = ([0xa0u8; 32], 0u32);
            let op_b = ([0xb0u8; 32], 0u32);
            let (cia, sa) = lp_add_kernel_fixture(1, &pid, &a, ra, founder, &csc, op_a, &[0x71u8; 32], seed_exp, &rax, &rab);
            let (cib, sb) = lp_add_kernel_fixture(1, &pid, &b, rb, founder, &csc, op_b, &[0x72u8; 32], seed_exp, &rbx, &rbb);
            let mut sc = ScanReflection::genesis();
            sc.fold_lp_add(
                1, &pid, &a, &b, ra, rb, founder, &csc, &[op_a], &[cia], &sa, &[op_b], &[cib], &sb,
                true, 0, 0, 0, 0, seed_exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, ep, ep,
            ).expect("POOL_INIT folds");
            sc
        };
        ...
        // (e) POOL_INIT EXACT-SHARE: a founder share_amount off by one is a malformed seed — instead of
        // self-burning it, the seeded delta_a / delta_b are returned as two owner-bound refund notes and NO pool
        // is registered (conservation, matching the variant-0 refund discipline).
        let ra = 1_000_000u64;
        let founder = (isqrt(ra as u128 * ra as u128) as u64) - AMM_MINIMUM_LIQUIDITY;
        let op_a = ([0xa0u8; 32], 0u32);
        let op_b = ([0xb0u8; 32], 0u32);
        let (cia, sa) = lp_add_kernel_fixture(1, &pid, &a, ra, founder + 1, &csc, op_a, &[0x71u8; 32], seed_exp, &rax, &rab);
        let (cib, sb) = lp_add_kernel_fixture(1, &pid, &b, ra, founder + 1, &csc, op_b, &[0x72u8; 32], seed_exp, &rbx, &rbb);
        let leaf_a = reflected_note_leaf(&a, &pedersen_commit_compressed(ra, &rab), &rax).unwrap();
        let mut acc = KeccakTreeAccumulator::new();
        let pa = acc.append_path();
        acc.append(&leaf_a);
        let pb = acc.append_path();
        ...
        assert!(sc.fold_lp_add(
            ...
            true, 0, 0, 0, 0, seed_exp, &rax, &rbx, &rab, &rbb, &rout_a, &rout_b, &pa, &pb,
        ).is_ok(), "POOL_INIT with a wrong founder share_amount refunds the seed");
    ...
    fn lp_remove_kernel_verify_and_fold_gates() {
        ...
        // The genesis note-append path: the positive cases below really append.
        let path = KeccakTreeAccumulator::new().append_path();
    ...
    // bridge_mint must authorize only against the bridge-burn set, not the all-spends
    ...
    fn bridge_burn_set_excludes_ordinary_spends() {
        ...
        assert_eq!(s.bridge_burns.get(&nu_ord), None, "ordinary spend NOT mintable");
