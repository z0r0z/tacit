// OP_ADAPTOR_LOCK box harness (not part of the crate build). Locks a note N into the lock-set (N and L open
// to the SAME amount — conservation, no change). FEE-LESS: a lock has no spendable output; the relay is
// recouped when the proceeds are later spent. Reads fixtures/adaptorlock_op.json. stdin order = the guest's
// OP_ADAPTOR_LOCK io::read (main.rs): header roots (spendRoot NON-zero: N membership; lockSetRoot 0 — lock
// APPENDS), then asset(32) ‖ locker(32) ‖ recipient(32) ‖ amount(u64) ‖ tx(32) ‖ ty(32) ‖ deadline(u64) ‖
// nCx(32) ‖ nCy(32) ‖ nIndex(u64) ‖ nPath[] ‖ nSigR(33) ‖ nSigZ(32) ‖ lCx(32) ‖ lCy(32) ‖ lSigR(33) ‖ lSigZ(32).
...
fn main() {
    let f: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(std::env::var("OP_FILE").unwrap_or_else(|_| "/root/work/cxfer/fixtures/adaptorlock_op.json".to_string())).unwrap()).unwrap();
