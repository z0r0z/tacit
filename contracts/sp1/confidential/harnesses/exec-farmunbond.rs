...
fn main() {
    ...
    stdin.write(&u64f(&f["fee"]).unwrap_or(0)); // relay fee carved from the released share (0 = self-settle), after shares
    ...
    if mode == "execute" {
        ...
        println!("EXECUTE_OK cycles={} pv_bytes={} shares={} fee={}",
            report.total_instruction_count(), pv.as_slice().len(), f["shares"], u64f(&f["fee"]).unwrap_or(0));
        return;
    }
