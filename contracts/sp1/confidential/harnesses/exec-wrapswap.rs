// OP_WRAP_SWAP box harness — 1-click swap from an external wallet.
//
// Consumes a pending public deposit as the swap input and mints the hidden output note in one settle
// (OP_SWAP fused with OP_WRAP). Like OP_WRAP_LP there is no tree note spent, so no membership, no nullifier
// and no change: the deposit's value is exact and public (bound into its deposit_id), which is what collapses
// wrap-then-swap into a single transaction and removes the intermediate note.
//
// OP_FILE mirrors the guest's read order exactly (main.rs OP_WRAP_SWAP): pair + fee tier, protocol-fee
// fields, reserves, direction, amount in, relay fee, the deposit (commitment, owner, opening sigma), min out,
// the minted output note + its sigma, then the deadline. `protocolFeeBps` MUST be 0 — the guest sends
