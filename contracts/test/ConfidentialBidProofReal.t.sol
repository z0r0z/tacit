...
/// Verifies a REAL SP1 Groth16 proof of the confidential OP_BID op ON-CHAIN, through the genuine
/// SP1VerifierGroth16 (v6.1.0) — no mock. The proof was GPU-proven on the box
/// (contracts/sp1/confidential/exec-bid.rs MODE=groth16) for the settle guest's OP_BID: a
/// buyer-offline partial-fill bid (buy up to 100 A at 5 B/unit, V_fund 500 B; a seller fills 40
/// from a 50 A note → 200 B to the seller, 40 A + 300 B refund to the buyer, 10 A seller change).
/// The buyer's per-fill openings are pre-signed offline; the op emits 2 input nullifiers + 4
/// output leaves and no swap/LP settlement. Fixture: contracts/test/fixtures/bid_groth16.json.
contract ConfidentialBidProofRealTest is Test {
    ...
    function setUp() public {
        ...
        string memory json = vm.readFile(string.concat(vm.projectRoot(), "/test/fixtures/bid_groth16.json"));
    ...
    /// The real BID proof verifies on-chain against the settle-guest vkey (reverts on failure).
    ...
    /// The proof commits the BID effect the pool's settle applies: 2 input nullifiers (funding +
    /// seller input) + 4 output leaves (buyer A, seller B pay, buyer B refund, seller A change),
    /// and no pool (swap/LP) settlement.
    function test_bid_effect_decodes() public view {
        ...
        assertEq(pv.nullifiers.length, 2, "two input nullifiers (funding + seller)");
        assertEq(pv.leaves.length, 4, "four output leaves (buyer A, seller B, buyer refund, seller change)");
