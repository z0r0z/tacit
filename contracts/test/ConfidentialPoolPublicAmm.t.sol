...
contract ConfidentialPoolPublicAmmTest is Test {
    ...
    function setUp() public {
        ...
        // Users approve the PERIPHERY; it pulls the input and forwards it to the pool.
    ...
    function test_public_amm_lifecycle() public {
        ...
        uint256 sh = amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 1_000_000, 4_000_000, 0, 0, address(this));
        ...
        uint256 out = amm.swapPublic(assetA, assetB, 30, 100_000, 0, 0, address(this));
        ...
        (uint256 la, uint256 lb) =
            amm.removeLiquidityPublicFrom(assetA, assetB, 30, sh, 0, 0, 0, address(this), address(this));
        ...
        // la/lb are canonical (lo, hi) amounts; map back to tokenA's leg for the balance check.
        uint256 aRet = assetA < assetB ? la : lb;
        assertGe(tokenA.balanceOf(address(this)) - aBefore, aRet, "A returned");
    }

    function test_remove_cannot_breach_min_liquidity() public {
        uint256 sh = amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 1_000_000, 1_000_000, 0, 0, address(this));
        // attempting to remove MORE than (totalShares - MIN) reverts
        vm.expectRevert(ConfidentialPool.InsufficientLiquidity.selector);
        amm.removeLiquidityPublicFrom(assetA, assetB, 30, sh + 1, 0, 0, 0, address(this), address(this));
    }
    ...
    function test_remove_liquidity_requires_owner_or_operator() public {
        ...
        vm.prank(operator);
        vm.expectRevert(ConfidentialPool.InsufficientLiquidity.selector);
        amm.removeLiquidityPublicFrom(assetA, assetB, 30, sh / 4, 0, 0, 0, address(this), recipient);

        amm.approveLpOperator(operator);
    ...
    function test_proportional_add_to_existing() public {
        ...
        uint256 sh2 = amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 500_000, 500_000, 0, 0, address(this));
    ...
    function test_public_amm_deadline_and_slippage_guards() public {
        ...
        amm.swapPublic(assetA, assetB, 30, 1000, 0, 999, address(this));
        vm.expectRevert(ConfidentialPool.Expired.selector);
        amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 1000, 1000, 0, 999, address(this));
        vm.expectRevert(ConfidentialPool.Expired.selector);
        amm.removeLiquidityPublicFrom(assetA, assetB, 30, 1000, 0, 0, 999, address(this), address(this));
        ...
        amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 500_000, 500_000, type(uint256).max, 0, address(this));
        ...
        vm.expectRevert(ConfidentialPool.SlippageExceeded.selector);
        amm.swapPublic(assetA, assetB, 30, 1000, type(uint256).max, 0, address(this));

        // sanity: the same actions succeed with satisfiable bounds + a future deadline
        uint256 outOk = amm.swapPublic(assetA, assetB, 30, 1000, 1, uint64(block.timestamp + 1), address(this));
    ...
    function test_public_amm_native_eth_value_accounting() public {
        ...
        amm.createPairAndAddLiquidityPublic{value: 1 ether - 1}(
            ethAsset, assetA, 30, 1 ether, 1_000_000, 0, 0, address(this)
        );

        uint256 sh = amm.createPairAndAddLiquidityPublic{value: 1 ether}(
            ethAsset, assetA, 30, 1 ether, 1_000_000, 0, 0, address(this)
        );
        ...
        vm.expectRevert(ConfidentialPool.EthValueMismatch.selector);
        amm.swapPublic{value: 1 ether}(assetA, ethAsset, 30, 1000, 0, 0, address(this));

        uint256 ethBefore = address(this).balance;
        uint256 outEth = amm.swapPublic(assetA, ethAsset, 30, 1000, 0, 0, address(this));
        ...
        uint256 outToken = amm.swapPublic{value: ethIn}(ethAsset, assetA, 30, ethIn, 0, 0, address(this));
    ...
    function test_public_amm_zero_recipient_reverts() public {
        ...
        amm.swapPublic(assetA, assetB, 30, 1000, 0, 0, address(0));

        vm.expectRevert(ConfidentialPool.ZeroAddress.selector);
        amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 1000, 1000, 0, 0, address(0));

        uint256 sh = pool.lpShares(_pid(assetA, assetB, 30), address(this));
        vm.expectRevert(ConfidentialPool.ZeroAddress.selector);
        amm.removeLiquidityPublicFrom(assetA, assetB, 30, sh / 4, 0, 0, 0, address(this), address(0));
    }
    ...
    function test_apply_applicators_reject_non_periphery() public {
        ...
        pool.applyPublicSwap(assetA, assetB, 30, 1000, 1, address(this));

        vm.expectRevert(ConfidentialPool.NotAuthorized.selector);
        pool.applyPublicAddLiquidity(lo, hi, 30, 1000, 1000, 1000, 1000, 1000, address(this));

        vm.expectRevert(ConfidentialPool.NotAuthorized.selector);
        pool.applyPublicRemoveLiquidity(assetA, assetB, 30, 1, address(this), address(this));
    }

    /// A public swap/add/remove routed through the periphery lands the SAME reserves/shares as the in-pool
    /// path did before the split — the applicator conservation gates accept exactly the AMM-correct deltas.
    function test_periphery_path_matches_expected_state() public {
        uint256 sh = amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 2_000_000, 2_000_000, 0, 0, address(this));
        bytes32 id = _pid(assetA, assetB, 30);
        assertEq(sh, 2_000_000 - 1000, "founder shares");
        (,,, uint256 rA, uint256 rB,, uint256 ts) = pool.pools(id);
        assertEq(rA, 2_000_000);
        assertEq(rB, 2_000_000);
        assertEq(ts, 2_000_000);

        // proportional add of half → mints ts/... exactly, reserves scale in ratio
        uint256 add = amm.createPairAndAddLiquidityPublic(assetA, assetB, 30, 1_000_000, 1_000_000, 0, 0, address(this));
        (,,, uint256 rA2, uint256 rB2,, uint256 ts2) = pool.pools(id);
        assertEq(add, ts / 2, "proportional shares");
        assertEq(rA2, 3_000_000);
        assertEq(rB2, 3_000_000);
        assertEq(ts2, ts + add);

        // swap: output equals the constant-product quote and k does not decrease
        uint256 vInG = 100_000 * (10000 - 30);
        uint256 quoted = (rB2 * vInG) / (rA2 * 10000 + vInG);
        uint256 out = amm.swapPublic(assetA, assetB, 30, 100_000, 0, 0, address(this));
        assertEq(out, quoted, "swap output == amm quote");
        (,,, uint256 rA3, uint256 rB3,,) = pool.pools(id);
        assertGe(rA3 * rB3, rA2 * rB2, "k non-decrease");
    }
}
